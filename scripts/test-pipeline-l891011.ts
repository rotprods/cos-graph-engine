...
import { PipelineL8L9L10L11, QueryIntent } from '../packages/graph/src/pipeline-l8l9l10l11';
import { KnowledgeGraphEngine, KGEntity, KGRelation } from '../packages/graph/src/level8-knowledge';
...
(function testPipelineCreation() {
  ...
  const pipe = new PipelineL8L9L10L11();
  assert(pipe instanceof PipelineL8L9L10L11, 'PipelineL8L9L10L11 is instantiated');
  assert(pipe.knowledgeGraph instanceof KnowledgeGraphEngine, 'Has L8 KnowledgeGraph');
  assert(pipe.semanticGraph instanceof SemanticGraph, 'Has L9 SemanticGraph');
  assert(pipe.embeddingGraph instanceof EmbeddingGraph, 'Has L10 EmbeddingGraph');
  assert(pipe.graphRAG instanceof GraphRAGEngine, 'Has L11 GraphRAG');

  // With custom options
  const pipe2 = new PipelineL8L9L10L11({ embeddingDim: 6, knnK: 2, autoBuildDemo: true });
  assert(pipe2 instanceof PipelineL8L9L10L11, 'Pipeline with custom options');
})();
...
(function testKnowledgeToSemantic() {
  ...
  assert(sem instanceof SemanticGraph, 'L9: Returns SemanticGraph');
...
(function testSemanticToEmbedding() {
  ...
  const pipe = new PipelineL8L9L10L11({ embeddingDim: 8, knnK: 2 });
  pipe.buildKnowledgeGraph(cosEntities, cosRelations);
  pipe.knowledgeToSemantic();
  const emb = pipe.semanticToEmbedding();

  assert(emb instanceof EmbeddingGraph, 'L10: Returns EmbeddingGraph');
...
(function testEmbeddingToGraphRAG() {
  ...
  assert(pipe.graphRAG instanceof GraphRAGEngine, 'L11: GraphRAG created');
...
(function testPipelineWithQuery() {
  ...
  const pipe = new PipelineL8L9L10L11({ embeddingDim: 8, knnK: 2 });
  const query: QueryIntent = { text: 'memory system', entities: ['memory'] };
...
(function testBuildDemo() {
  ...
  const pipe = new PipelineL8L9L10L11({ embeddingDim: 8 });
...
(function testSingleEntity() {
  ...
  const pipe = new PipelineL8L9L10L11();
  pipe.buildKnowledgeGraph(
    [{ id: 'a', name: 'Alpha', type: 'concept', description: 'First entity' }],
    []
  );
  pipe.knowledgeToSemantic();
  pipe.semanticToEmbedding();
...
(function testAccessors() {
  ...
  assert(pipe.getKnowledgeGraph() instanceof KnowledgeGraphEngine, 'Access: getKnowledgeGraph');
  assert(pipe.getSemanticGraph() instanceof SemanticGraph, 'Access: getSemanticGraph');
  assert(pipe.getEmbeddingGraph() instanceof EmbeddingGraph, 'Access: getEmbeddingGraph');
  assert(pipe.getGraphRAG() instanceof GraphRAGEngine, 'Access: getGraphRAG');
})();
...
(function testAnswerQuery() {
  ...
  const query: QueryIntent = { text: 'reasoning', entities: ['reasoning'] };
...
(function testSerialization() {
  ...
  // Deserialize
  const kg2 = KnowledgeGraphEngine.fromJSON(kgJSON);
  const sem2 = SemanticGraph.fromJSON(semJSON);
  const emb2 = EmbeddingGraph.fromJSON(embJSON);
