...
} from '../packages/execution/src/authority-phase05-platform-v2';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    check(Buffer.from(firstRead.bodyBase64, 'base64').toString('utf8') === 'original', 'read begins at explicit offset zero regardless of existing file position');
    ...
    check(write.bytesWritten === 7 && write.position === 0, 'write loops from explicit position and syncs');
    check(await readFile(renamedPath, 'utf8') === 'updated', 'renamed inode is mutated through handle without path reopen');
    ...
    check(append.position === 7 && append.bytesWritten === 5, 'append resolves current file size inside serialized handle queue');
    check(await readFile(renamedPath, 'utf8') === 'updated-tail', 'append writes at deterministic end position');

    const secondRead = await executor.execute(request('read')) as { bodyBase64: string };
    check(Buffer.from(secondRead.bodyBase64, 'base64').toString('utf8') === 'updated-tail', 'subsequent read still begins at offset zero');

    const stat = await executor.execute(request('stat')) as { isFile: boolean; size: number };
    check(stat.isFile && stat.size === 12, 'stat consumes the same registered handle');
  ...
  console.log(`Authority Node file-handle executor V2 contract: ${assertions} assertions passed`);
}
...
function request(operation: string, payload?: unknown): AuthorityFileHandleExecutionRequest {
  ...
  return {
    ...
    ...(operation === 'read' || operation === 'stat'
      ? {}
      : { providerIdempotencyKey: 'provider-file-v1' }),
    context: {
