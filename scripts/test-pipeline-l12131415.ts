...
import { PipelineL12L13L14L15 } from '../packages/graph/src/pipeline-l12l13l14l15';
...
(function testPipelineCreation() {
  console.log('\n=== Pipeline Creation ===');
  const pipe = new PipelineL12L13L14L15();
  assert(pipe instanceof PipelineL12L13L14L15, 'Pipeline instantiated');
  assert(pipe.memoryGraph instanceof MemoryGraphEngine, 'Has L12 Memory');
  assert(pipe.agentGraph instanceof AgentGraphEngine, 'Has L13 Agent');
  assert(pipe.toolGraph instanceof ToolGraphEngine, 'Has L14 Tool');
  assert(pipe.workflowGraph instanceof WorkflowGraphEngine, 'Has L15 Workflow');
})();
...
(function testAccessors() {
  ...
  assert(pipe.getMemoryGraph() instanceof MemoryGraphEngine, 'Access: getMemoryGraph');
  assert(pipe.getAgentGraph() instanceof AgentGraphEngine, 'Access: getAgentGraph');
  assert(pipe.getToolGraph() instanceof ToolGraphEngine, 'Access: getToolGraph');
  assert(pipe.getWorkflowGraph() instanceof WorkflowGraphEngine, 'Access: getWorkflowGraph');
})();
