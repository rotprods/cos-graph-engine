...
async function main(): Promise<void> {
  ...
  const appliedResult = await runtime.recoverInterrupted({
    ...
    reconciler: new AuthorityProviderReconciler({
      inspectedAt: at(5), inspection: appliedInspector,
    }),
  });
  check(appliedResult.disposition === 'committed', 'provider-applied inspection commits the existing operation');
  check(appliedResult.operation.effectKnowledge === 'applied', 'applied recovery records provider truth');
  check(appliedInspector.requests.length === 1, 'recovery performs one read-only provider inspection');
  check(appliedInspector.requests[0]?.providerIdempotencyKey === 'provider-applied-v1', 'inspection uses durable provider identity');
  check(appliedInspector.requests[0]?.target.kind === 'http', 'HTTP target is extracted without repeating mutation');
  ...
  const absentResult = await runtime.recoverInterrupted({
    ...
    reconciler: new AuthorityProviderReconciler({
      ...
      inspection: new FakeInspector({
        status: 'not_applied', authoritativeAbsence: true,
        evidence: { providerLookup: 'authoritative-ledger', result: 'not-found' },
      }),
      retryPlanner,
    }),
  });
  check(absentResult.disposition === 'prepared_for_retry', 'authoritative absence prepares a new attempt');
  check(absentResult.operation.state === 'prepared' && absentResult.operation.attempt === 2, 'retry is prepared, never executing');
  check(absentResult.operation.fencingToken === 11, 'retry receives a newer fencing token');
  check(absentResult.operation.providerIdempotencyKey === 'provider-absent-v2', 'retry rotates provider identity');
    ...
    reconciler: new AuthorityProviderReconciler({
      ...
      inspection: new FakeInspector({
        ...
        error: {
          ...
          message: 'Primary exists but secondary index is missing',
          retryable: false,
  ...
  check(partialResult.disposition === 'compensation_required', 'partial application is never represented as absence');
  check(partialResult.operation.state === 'compensation_required', 'partial recovery requires compensation');
  ...
  await assert.rejects(() => runtime.recoverInterrupted({
    ...
    reconciler: new AuthorityProviderReconciler({
      inspectedAt: at(35),
      inspection: new FakeInspector({
        status: 'unknown', reason: 'provider inspection timed out',
        evidence: { timeoutMs: 3000 },
      }),
    }),
  }), /PROVIDER_RECONCILIATION_INCONCLUSIVE/);
  assertions += 1;
  check((await runtime.get(unknown.operationId))?.state === 'reconciliation_required', 'inconclusive evidence leaves operation uncertain');
  ...
  assertions += 1;
  check((await runtime.get(badFence.operationId))?.state === 'reconciliation_required', 'invalid retry plan cannot mutate operation');

  console.log(`Authority provider reconciliation clean contract: ${assertions} assertions passed`);
}
...
async function createUnknownOperation(
  ...
  const claimed = await runtime.claim({
    ...
    causationId: null,
    provenance: [{ source: `test://phase05e/provider-reconciliation/${suffix}` }],
    metadata: {},
...
class StaticRetryPlanner implements AuthorityProviderRetryPlanner {
  ...
  async planRetry(_request: AuthorityProviderRetryPlanRequest) {
    return { nextFencingToken: this.token, nextProviderIdempotencyKey: this.providerKey, evidence: {} };
  }
}
