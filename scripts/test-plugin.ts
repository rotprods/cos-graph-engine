...
import { PluginRegistry, PluginMarketplace, PluginSystem, pluginSystem, Plugin, HookName, GraphData } from '../packages/graph/src/plugin';
...
assert(pluginSystem !== undefined, 'pluginSystem singleton exists');
assert(pluginSystem.registry.list().length >= 5, 'Singleton registry has 5+ plugins');
