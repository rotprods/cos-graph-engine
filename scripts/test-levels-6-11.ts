...
async function main() {
  ...
  // L6: Data Flow
  const df = new DataFlowGraph(); df.buildMLPipeline();
  ...
  // L8: Knowledge Graph
  const kg = new KnowledgeGraphEngine(); kg.buildAIEcosystem();
  ...
  // L9: Semantic Graph
  const sg = new SemanticGraph(); sg.buildAnimalTaxonomy();
  ...
  // L10: Embedding Graph
  const eg = new EmbeddingGraph(); eg.buildAIModelGraph();
