...
/** Strict SQL fixture for AuthoritySideEffectPostgresStore. */
export class FakeAuthoritySideEffectStorePostgres implements PostgresExecutor {
  ...
  async transaction<T>(fn: (tx: PostgresTransaction) => Promise<T>): Promise<T> {
    const operationSnapshot = structuredClone(this.operations);
    const revisionSnapshot = structuredClone(this.revisions);
    try {
      return await fn({ query: (sql, params) => this.queryInternal(sql, params) });
    } catch (error) {
      this.operations = operationSnapshot;
      this.revisions = revisionSnapshot;
      throw error;
    }
  }
  ...
  corruptRevision(
    revisionId: string,
    mutate: (row: AuthoritySideEffectRevisionRow) => void,
  ): void {
  ...
  private async queryInternal<Row>(
    ...
    if (statement.includes('from cos_execution.authority_side_effect_operations o')
      ...
      const projectId = String(params[0]);
      const key = String(params[1]);
      const operation = this.operations.find(row =>
        row.project_id === projectId && row.idempotency_key === key);
  ...
  private insertOperation<Row>(params: unknown[]): PostgresQueryResult<Row> {
    ...
    const duplicate = this.operations.find(existing =>
      existing.project_id === row.project_id
      && existing.idempotency_key === row.idempotency_key);
    if (duplicate) return result<Row>([]);
    if (this.operations.some(existing => existing.operation_id === row.operation_id)) {
      return result<Row>([]);
    }
...
function rowFromParams(params: unknown[]): AuthoritySideEffectRevisionRow {
  return {
    ...
    error_value: params[22] === null
      ? null
      : parseJson(params[22], 'error_value') as AuthoritySideEffectRevisionRow['error_value'],
    error_hash: nullableString(params[23]),
    compensation_value: params[24] === null
      ? null
      : parseJson(params[24], 'compensation_value') as AuthoritySideEffectRevisionRow['compensation_value'],
    correlation_id: String(params[25]),
    causation_id: nullableString(params[26]),
    provenance: parseJson(params[27], 'provenance') as AuthoritySideEffectRevisionRow['provenance'],
