...
type TicketStatus = 'diseniando' | 'planificado' | 'ejecutando' | 'validando'
  | 'testeando' | 'refactorizando' | 'debuggeando' | 'completado' | 'bloqueado';

type TicketType = 'feature' | 'bugfix' | 'performance' | 'documentation'
  | 'testing' | 'infra' | 'cleanup';
...
const STATE_FILE = join(LOOP_DIR, 'state.json');
const BOARD_FILE = join(LOOP_DIR, 'board.json');
...
function getTicketsFase4(): Ticket[] {
  return [
...
function getTicketsFase5(): Ticket[] {
  return [
...
function getTicketsFase6(): Ticket[] {
  return [
  ...
  function getTicketsFase3(): Ticket[] {
  return [
    {
...
function cmdStatus(): void {
  ...
  if (!fase) { console.error('Fase no encontrada'); return; }
...
function cmdStart(ticketId: string): void {
  const state = loadState();
  const ticket = state.tickets.find(t => t.id === ticketId);
  if (!ticket) { console.error(`Ticket ${ticketId} no encontrado`); return; }
...
function cmdCommit(ticketId: string): void {
  const state = loadState();
  const ticket = state.tickets.find(t => t.id === ticketId);
  if (!ticket) { console.error(`Ticket ${ticketId} no encontrado`); return; }
...
function cmdBoard(): void {
  const state = loadState();

  const columns: TicketStatus[] = ['diseniando', 'planificado', 'ejecutando', 'validando',
    'testeando', 'refactorizando', 'debuggeando', 'completado', 'bloqueado'];
  ...
  for (const t of allTickets) {
    ...
    if (saved) t.estado = saved.estado;
  }

  for (const col of columns) {
    const items = allTickets.filter(t => t.estado === col);
    if (items.length === 0 && col !== 'completado') continue;
...
function cmdPhase(faseId: number): void {
  const fase = FASES.find(f => f.id === faseId);
  if (!fase) { console.error(`Fase ${faseId} no encontrada`); return; }
  ...
  if (ticketMap[faseId]) {
    for (const t of ticketMap[faseId]()) {
      if (!allTickets.find(st => st.id === t.id)) allTickets.push(t);
    }
  }
...
function cmdCheck(): void {
  ...
  for (const check of checks) {
    ...
    try {
      const output = execSync(check.cmd, { encoding: 'utf-8', timeout: 120000 });
...
function main(): void {
  ...
  switch (cmd) {
    case 'status':
      ...
      break;
    case 'start':
      if (!args[1]) { console.error('Uso: npx tsx scripts/loop.ts start <ticket-id>'); return; }
      cmdStart(args[1]);
      break;
    case 'commit':
      if (!args[1]) { console.error('Uso: npx tsx scripts/loop.ts commit <ticket-id>'); return; }
      cmdCommit(args[1]);
      break;
    case 'board':
      cmdBoard();
      break;
    case 'phase':
      if (!args[1]) { console.error('Uso: npx tsx scripts/loop.ts phase <N>'); return; }
