...
async function main() {
  ...
  await check('Cognition coverage is deterministic across integration order', async () => {
  ...
  const receipt = {
    ...
    semantics: 'W1C missing/superseded checks carried forward through portable module resolution; W1D authority wins on ToolRegistry; cognition coverage made deterministic across test order',
    checks,
