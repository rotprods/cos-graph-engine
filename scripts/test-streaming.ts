...
import {
  GraphPatch, GraphPatchBuilder, PatchSerializer,
  GraphStream, Observable, GraphObserver, SubscriptionManager,
  PatchFilter, Connection, Subscription,
} from '../packages/graph/src/streaming';
...
const p1 = GraphPatchBuilder.nodeAdded(5, 'g1', 'n1', { label: 'test' });
...
const p2 = GraphPatchBuilder.nodeRemoved(3, 'g2', 'n2');
...
const json = PatchSerializer.serialize(p1);
assert(typeof json === 'string', 'serialize returns string');
const deserialized = PatchSerializer.deserialize(json);
...
try {
  PatchSerializer.deserialize('{}');
  assert(false, 'Should throw on empty object');
} catch (e) {
  assert((e as Error).message.includes('missing required fields'), 'Empty object throws');
}

try {
  PatchSerializer.deserialize('{"id":"x","type":"invalid","level":0,"graphId":"g"}');
  assert(false, 'Should throw on invalid type');
} catch (e) {
  assert((e as Error).message.includes('Invalid patch type'), 'Invalid type throws');
}

section('PatchSerializer — batch serialize/deserialize');

const batch = [p1, p2, p3];
const batchJson = PatchSerializer.serializeBatch(batch);
assert(typeof batchJson === 'string', 'batch serialize returns string');
const batchBack = PatchSerializer.deserializeBatch(batchJson);
...
const updates = [
  GraphPatchBuilder.nodeUpdated(5, 'g1', 'n1', { a: 1 }),
  GraphPatchBuilder.nodeUpdated(5, 'g1', 'n1', { b: 2 }),
  GraphPatchBuilder.nodeUpdated(5, 'g1', 'n1', { c: 3 }),
  GraphPatchBuilder.nodeRemoved(5, 'g1', 'n2'),
];
const compressed = PatchSerializer.compress(updates);
...
const stream = new GraphStream(100);
...
const pLater = GraphPatchBuilder.nodeAdded(9, 'g10', 'n10');
...
const received: GraphPatch[] = [];
const cleanup = stream.subscribe((patch) => { received.push(patch); });
stream.sendPatch(GraphPatchBuilder.nodeAdded(1, 'g-test', 'n-test'));
...
const edgesReceived: GraphPatch[] = [];
const cleanup2 = stream.onPatchType('edge_added', (patch) => { edgesReceived.push(patch); });
stream.sendPatch(GraphPatchBuilder.edgeAdded(4, 'g-edge', 'e-edge'));
...
const unsubEvents: string[] = [];
const cb = (data: string) => { unsubEvents.push(data); };
...
const { observer, emitter } = GraphObserver.createStandalone();
...
emitter.emit(GraphPatchBuilder.nodeAdded(3, 'g', 'node-A'));
emitter.emit(GraphPatchBuilder.nodeAdded(3, 'g', 'node-B'));
...
emitter.emit(GraphPatchBuilder.nodeRemoved(3, 'g', 'node-A'));
...
emitter.emit(GraphPatchBuilder.edgeAdded(4, 'g', 'edge-X'));
...
emitter.emit(GraphPatchBuilder.edgeRemoved(4, 'g', 'edge-X'));
...
emitter.emit(GraphPatchBuilder.nodeUpdated(5, 'g', 'node-C', { val: 99 }));
...
emitter.emit(GraphPatchBuilder.stateChanged(0, 'g-state', { status: 'ready' }));
...
const levelEvents: GraphPatch[] = [];
observer.observeLevel(3, (patch) => { levelEvents.push(patch); });
emitter.emit(GraphPatchBuilder.nodeAdded(3, 'g', 'n-L3'));
emitter.emit(GraphPatchBuilder.nodeAdded(7, 'g', 'n-L7'));
...
const graphEvents: GraphPatch[] = [];
observer.observeGraph('g-target', (patch) => { graphEvents.push(patch); });
emitter.emit(GraphPatchBuilder.nodeAdded(1, 'g-target', 'n-t'));
emitter.emit(GraphPatchBuilder.nodeAdded(1, 'g-other', 'n-o'));
...
const filteredEvents: GraphPatch[] = [];
observer.observe({ types: ['node_added', 'edge_added'], levels: [1] }, (patch) => {
  filteredEvents.push(patch);
});
emitter.emit(GraphPatchBuilder.nodeAdded(1, 'g', 'n-f1'));
emitter.emit(GraphPatchBuilder.edgeAdded(1, 'g', 'e-f1'));
emitter.emit(GraphPatchBuilder.nodeRemoved(1, 'g', 'n-f1')); // Should not fire
emitter.emit(GraphPatchBuilder.nodeAdded(9, 'g', 'n-f9')); // Level 9 should not fire
...
emitter.emit(GraphPatchBuilder.nodeAdded(1, 'g', 'n-u1'));
observer.unsubscribe(sub.id);
emitter.emit(GraphPatchBuilder.nodeAdded(1, 'g', 'n-u2'));
...
emitter.emit(GraphPatchBuilder.nodeAdded(1, 'g', 'n-u3'));
...
emitter.emit(GraphPatchBuilder.nodeAdded(1, 'g', 'n-u4'));
...
const stream2 = new GraphStream(100);
...
stream2.sendPatch(GraphPatchBuilder.nodeAdded(2, 'g2', 'n-stream'));
stream2.sendPatch(GraphPatchBuilder.edgeRemoved(2, 'g2', 'e-stream'));
stream2.sendPatch(GraphPatchBuilder.nodeAdded(2, 'g2', 'n-stream2'));
...
const sub1: Subscription = { id: 's1', label: 'test1', filter: {}, active: true, createdAt: Date.now(), callCount: 0 };
const sub2: Subscription = { id: 's2', label: 'test2', filter: { types: ['node_added'] }, active: true, createdAt: Date.now(), callCount: 0 };
