...
async function main(): Promise<void> {
  ...
  const input = {
    ...
    correlationId: 'corr-repair-pg', kind: 'lease_release' as const,
    ...
    sensitivity: 'private' as const, maxAttempts: 3, nextAttemptAt: at(0),
  ...
  const duplicate = await service.enqueue({ ...input, idempotencyKey: 'different-enqueue-transport-key' });
  ...
  check(!db.statements.some(sql => /^UPDATE\b|^DELETE\b|^TRUNCATE\b/i.test(sql)), 'repair adapter never updates or deletes history');
  check(db.statements.some(sql => sql.includes('pg_advisory_xact_lock')), 'repair writer uses advisory lock');
