...
const REPORT_PATH = join(__dirname, '..', 'docs', 'benchmark-report.html');
...
function loadResults(file: string): BenchResult[] {
  ...
  if (!existsSync(path)) { console.log(`  ⚠ ${file} not found`); return []; }
...
// Group by level
const byLevel = new Map<string, BenchResult[]>();
...
function getScale(n: number): string {
  if (n <= 30) return 'n=10';
  if (n <= 200) return 'n=100';
  return 'n=1000';
}

// Build "addNode" comparison across levels at n=1000
const addNodeN1000: {level: string; ms: number}[] = [];
const addEdgeN1000: {level: string; ms: number}[] = [];
const validateN1000: {level: string; ms: number}[] = [];
const serializeN1000: {level: string; ms: number}[] = [];
...
for (const level of levelOrder) {
  ...
  if (addN > 0) addNodeN1000.push({level, ms: addN});
  if (addE > 0) addEdgeN1000.push({level, ms: addE});
  if (val > 0) validateN1000.push({level, ms: val});
  if (ser > 0) serializeN1000.push({level, ms: ser});
}

function tableRows(arr: {level: string; ms: number}[]): string {
...
function scatterData(arr: {level: string; ms: number}[]): string {
