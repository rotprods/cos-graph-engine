...
await check('authority projection is deterministic and scope-safe', () => {
  ...
  const reportA = first.project(registry, {
    version: 1,
    scope: { projectId: 'P-001', maxSensitivity: 'internal', asOf: '2026-08-24T11:00:00.000Z' },
    sourceCursor: '1',
  });
  const reportB = second.project(registry, {
    version: 1,
    scope: { projectId: 'P-001', maxSensitivity: 'internal', asOf: '2026-08-24T11:00:00.000Z' },
    sourceCursor: '1',
  });
...
if (failed.length) {
  console.error(`W13 authority contract failed: ${failed.length}/${results.length}`);
  process.exit(1);
}
