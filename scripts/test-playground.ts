...
import {
  LevelPlayground, PlaygroundSession,
  Tutorial, TutorialRegistry, TutorialRunner,
} from '../packages/graph/src/playground';
...
const pg0 = new LevelPlayground(0);
...
for (let l = 0; l <= 19; l++) {
  const pg = new LevelPlayground(l);
...
try {
  new LevelPlayground(99);
  assert(false, 'Should throw on invalid level');
} catch (e) {
  assert((e as Error).message.includes('not found'), 'Invalid level throws');
}
...
const pg1 = new LevelPlayground(1);
...
const pg0v = new LevelPlayground(0);
pg0v.execute('create');
pg0v.execute('add A');
pg0v.execute('add B');
...
const steps = [
  { id: 's1', title: 'Step 1', description: 'Do this', expectedCommand: 'create', hint: 'Use create', validate: (i: string) => ({ passed: i.includes('create'), message: 'Type create' }) },
  { id: 's2', title: 'Step 2', description: 'Do that', expectedCommand: 'add', hint: 'Use add', validate: (i: string) => ({ passed: i.includes('add'), message: 'Type add' }) },
];
const tutorial = new Tutorial('T1', 0, 'Test Tutorial', 'A test tutorial', steps);
