...
} from '../packages/execution/src/authority-phase05-observed';
...
async function main(): Promise<void> {
  ...
  await assert.rejects(async () => { store.append(tamperedHash); }, /SIGNAL_ID_MISMATCH|SIGNAL_HASH_MISMATCH/);
  assertions += 1;

  const tamperedId = structuredClone(first);
  tamperedId.signalId = 'capsig_tampered';
  await assert.rejects(async () => { store.append(tamperedId); }, /SIGNAL_ID_MISMATCH/);
  assertions += 1;
