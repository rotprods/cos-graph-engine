...
// Test 1: Constructor
const executor = new PruningExecutor([
  new MaxDepthPruning(10),
  new VisitedPruning(),
]);
assert(executor !== undefined, 'PruningExecutor constructed');

// Test 2: shouldPrune with MaxDepth
const state1 = createPruningState('A', 'Z', 3);
state1.depth = 0;
...
const executor2 = new PruningExecutor([new VisitedPruning()]);
const state2 = createPruningState('A', 'Z', 10);
state2.depth = 0;
...
// Test 4: Reset
executor2.reset();
const state3 = createPruningState('A', 'Z', 10);
state3.depth = 0;
...
const executor3 = new PruningExecutor([new MaxDepthPruning(2)]);
const state4 = createPruningState('X', 'Z', 10);
...
const expandTracker: string[] = [];
const strategyWithHook = new (class implements any {
  readonly name = 'HookTest';
  shouldPrune() { return false; }
  onExpand(nodeId: string) { expandTracker.push(nodeId); }
  reset() {}
})();

const executor4 = new PruningExecutor([strategyWithHook]);
const state5 = createPruningState('A', 'Z', 10);
state5.depth = 0;
...
const strategyWithTarget = new (class implements any {
  readonly name = 'TargetHook';
  shouldPrune() { return false; }
  onTargetFound() { targetFoundCalled = true; }
  reset() {}
})();

const executor5 = new PruningExecutor([strategyWithTarget]);
executor5.onTargetFound('Z', createPruningState('A', 'Z', 10));
...
const executor6 = new PruningExecutor([
  new MaxDepthPruning(1),  // prunea depth >= 1
  new (class implements any {
    readonly name = 'NeverCalled';
    shouldPrune() { throw new Error('Should not be called after short-circuit'); }
    reset() {}
  })(),
]);
const state6 = createPruningState('A', 'Z', 10);
state6.depth = 5;
...
const maxDepth = new MaxDepthPruning(5);
assert(!maxDepth.shouldPrune('A', 0, createPruningState('A')), 'depth 0 < 5');
assert(!maxDepth.shouldPrune('A', 4, createPruningState('A')), 'depth 4 < 5');
assert(maxDepth.shouldPrune('A', 5, createPruningState('A')), 'depth 5 == maxDepth');
assert(maxDepth.shouldPrune('A', 10, createPruningState('A')), 'depth 10 > 5');

const maxDepthInf = new MaxDepthPruning(Infinity);
assert(!maxDepthInf.shouldPrune('A', 1000, createPruningState('A')), 'Infinity = no limit');

const maxDepthZero = new MaxDepthPruning(0);
assert(maxDepthZero.shouldPrune('A', 0, createPruningState('A')), 'depth 0 >= maxDepth 0');
...
const visited = new VisitedPruning();
assert(!visited.shouldPrune('A', 0, createPruningState('A')), 'first visit A not pruned');
assert(visited.shouldPrune('A', 1, createPruningState('A')), 'second visit A pruned');
assert(!visited.shouldPrune('B', 0, createPruningState('A')), 'first visit B not pruned');

visited.reset();
assert(!visited.shouldPrune('A', 0, createPruningState('A')), 'after reset, A not pruned');
...
const dirPruning = new TargetDirectionPruning(dirGraph, 'D', 5);
const stateDir = createPruningState('A', 'D', 10);
...
const dirPruning2 = new TargetDirectionPruning(dirGraph, 'Z', 5);
stateDir.currentNode = 'A';
assert(!dirPruning2.shouldPrune('A', 0, stateDir), 'no ancestors = no pruning');

// Empty graph
const emptyGraph = new CSRGraph();
const dirPruning3 = new TargetDirectionPruning(emptyGraph, 'D', 5);
stateDir.currentNode = 'A';
assert(!dirPruning3.shouldPrune('A', 0, stateDir), 'empty graph = no pruning');

// Self-loop
const selfGraph = new CSRGraph();
selfGraph.addNode({ id: 'A' });
selfGraph.addEdge('A', 'A');
const dirPruning4 = new TargetDirectionPruning(selfGraph, 'A', 5);
stateDir.currentNode = 'A';
...
const costBound = new CostBoundPruning();
const stateCost = createPruningState('A', 'Z', 10);
...
const beam1 = new BeamPruning(1); // Beam width = 1
const stateBeam = createPruningState('A', 'Z', 10);
...
const beam5 = new BeamPruning(5);
const stateBeam2 = createPruningState('A', 'Z', 10);
for (let i = 0; i < 5; i++) {
  stateBeam2.depth = 0;
...
beam5.reset();
const stateBeam3 = createPruningState('A', 'Z', 10);
...
const lmGraph = new CSRGraph();
for (const id of ['A', 'B', 'C', 'D', 'E', 'F']) lmGraph.addNode({ id });
lmGraph.addEdge('A', 'B'); lmGraph.addEdge('B', 'C');
lmGraph.addEdge('C', 'D'); lmGraph.addEdge('D', 'E');
lmGraph.addEdge('E', 'F');

const landmark = new LandmarkPruning(lmGraph, ['A', 'F'], 2);
const stateLm = createPruningState('A', 'F', 10);

stateLm.currentNode = 'A';
assert(!landmark.shouldPrune('A', 0, stateLm), 'landmark: source not pruned');

// No target = no pruning
const stateLm2 = createPruningState('A', undefined, 10);
stateLm2.currentNode = 'A';
assert(!landmark.shouldPrune('A', 0, stateLm2), 'no target = no pruning');

// Empty landmarks
const emptyLm = new LandmarkPruning(lmGraph, [], 2);
stateLm.currentNode = 'A';
...
const earlyExit = new EarlyExitPruning();
const stateExit = createPruningState('A', 'Z', 10);
...
// Target = source
const earlyExit2 = new EarlyExitPruning();
const stateExit2 = createPruningState('A', 'A', 10);
...
const { nodes: bfsNodes, result: bfsResult } = manualBFS(testGraph, 'A', [
  new MaxDepthPruning(3),  // depth < 3 → A(0), B(1), C(1), D(2), E(2), F(2), G(2) = 7 nodes
  new VisitedPruning(),
]);
...
const discGraph = new CSRGraph();
discGraph.addNode({ id: 'A' });
discGraph.addNode({ id: 'B' });
const { path: discPath } = manualBiBFS(discGraph, 'A', 'B', [new VisitedPruning()], 10);
...
function manualBFS(
  ...
  strategies: any[],
  maxDepth: number = 10
): { nodes: Array<{ id: string; depth: number }>; result: any } {
  const executor = new PruningExecutor(strategies);
  const state = createPruningState(source, undefined, maxDepth);
  ...
  while (queue.length > 0) {
    ...
    if (executor.shouldPrune(current.id, current.depth, state)) continue;
...
function manualBiBFS(
  graph: CSRGraph,
  source: string,
  target: string,
  strategies: any[],
  maxDepth: number = 20
): { path: Array<{ id: string; depth: number }> | null; result: any } {
  const executor = new PruningExecutor(strategies);
  const state = createPruningState(source, target, maxDepth);
  ...
  while (fIdx < fQueue.length || bIdx < bQueue.length) {
    ...
    if (fIdx < fQueue.length) {
      ...
      if (executor.shouldPrune(cur.id, cur.depth, state)) continue;
...
async function runLevelTests() {
  class MockLevel {
    ...
    constructor(name: string, graph: CSRGraph) {
      this.name = name;
      this.graph = graph;
    }

    async traverse(source: string, options?: { strategies?: any[]; maxDepth?: number }) {
      const strategies = options?.strategies || [new VisitedPruning()];
      const maxDepth = options?.maxDepth || 10;
      return manualBFS(this.graph, source, strategies, maxDepth);
    }

    async shortestPath(source: string, target: string, options?: { strategies?: any[]; maxDepth?: number }) {
      const strategies = options?.strategies || [new EarlyExitPruning(), new VisitedPruning()];
      const maxDepth = options?.maxDepth || 20;
      return manualBiBFS(this.graph, source, target, strategies, maxDepth);
    }
  }

  const knowledgeGraph = new CSRGraph();
  for (let i = 0; i < 50; i++) knowledgeGraph.addNode({ id: `entity_${i}` });
  for (let i = 0; i < 10; i++) {
    for (let j = 1; j <= 4; j++) {
      const child = i * 4 + j;
      if (child < 50) knowledgeGraph.addEdge(`entity_${i}`, `entity_${child}`);
    }
  }
  ...
  const { path: kPath } = await level.shortestPath('entity_0', 'entity_20', {
    strategies: [new LandmarkPruning(knowledgeGraph, ['entity_0', 'entity_25', 'entity_49'], 2), new EarlyExitPruning(), new VisitedPruning()],
    maxDepth: 10,
  });
