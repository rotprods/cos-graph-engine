...
} from '../packages/execution/src/authority-phase05-provider-integrations';
...
async function main(): Promise<void> {
  ...
  check(appliedTransport.calls[0]?.target.method === 'GET', 'inspector never invokes the original POST mutation');
  check(appliedTransport.calls[0]?.providerIdempotencyKey === request.providerIdempotencyKey, 'durable provider identity reaches status transport');
  check(appliedTransport.calls[0]?.headers?.['x-cos-provider-idempotency-key'] === request.providerIdempotencyKey, 'provider identity is explicit in inspection header');
