...
start = text.index('const LEVEL_ENGINES:')
end = text.index('\n\nfunction generateHTML', start)
adapter_block = r'''interface VisualizerLevelSnapshot {
  nodes: any[];
  edges: any[];
  metrics: any;
}
...
text = text[:start] + adapter_block + text[end:]

gen_start = text.index('export function generateVisualizer(')
gen_end = text.index('\nexport function generateVisualizerFile(', gen_start)
generator = r'''export function generateVisualizer(
  levels?: string[],
...
'''
text = text[:gen_start] + generator + text[gen_end:]
p.write_text(text)
