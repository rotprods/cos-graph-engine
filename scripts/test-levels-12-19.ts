...
let p = 0, f = 0;
function assert(c: boolean, m: string) { if (c) { p++; console.log('  ✅ ' + m); } else { f++; console.log('  ❌ ' + m); } }

async function main() {
  console.log('📊 Levels 12-19 Tests\n');

  // L12: Memory Graph
  const mem = new MemoryGraphEngine('Test Memory');
  ...
  const restored = MemoryGraphEngine.fromJSON(json);
  ...
  // Empty graph
  const empty = new MemoryGraphEngine('Empty');
  ...
  // L13: Agent Graph
  const agents = new AgentGraphEngine('Test Agents');
  agents.buildDevTeam();
  ...
  // Empty graph
  const emptyA = new AgentGraphEngine();
  assert(emptyA.findByCapability('code').length === 0, 'L13: Empty findByCapability');

  // L14: Tool Graph
  const tools = new ToolGraphEngine('Test Tools');
  ...
  // L15: Workflow Graph
  const wf = new WorkflowGraphEngine('Support Flow', 'Customer support automation');
  ...
  // Cycle detection
  const wf2 = new WorkflowGraphEngine('Cyclic');
  ...
  // L16: Network Graph
  const net = new NetworkGraphEngine('Test Net');
  ...
  // L17: Social Graph
  const soc = new SocialGraphEngine('Test Social');
  ...
  const bio = new BiologicalGraphEngine('Test Neural');
  ...
  // Protein network
  const ppi = new BiologicalGraphEngine('PPI');
  ...
  // L19: Molecular Graph
  const mol = new MolecularGraphEngine('Water');
  mol.buildWater();
  ...
  // Benzene
  const benz = new MolecularGraphEngine('Benzene');
  benz.buildBenzene();
  ...
  // Aspirin
  const aspirin = new MolecularGraphEngine('Aspirin');
  ...
  // Empty graph
  const emptyM = new MolecularGraphEngine('Empty');
