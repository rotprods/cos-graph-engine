...
export interface AuthoritySideEffectRevisionRow {
  revision_id: string;
  operation_id: string;
  project_id: string;
  idempotency_key: string;
...
export const AUTHORITY_SIDE_EFFECT_POSTGRES_DDL = `
CREATE SCHEMA IF NOT EXISTS cos_execution;

CREATE TABLE IF NOT EXISTS cos_execution.authority_side_effect_operations (
...
CREATE TABLE IF NOT EXISTS cos_execution.authority_side_effect_revisions (
  revision_id TEXT PRIMARY KEY,
  operation_id TEXT NOT NULL REFERENCES cos_execution.authority_side_effect_operations(operation_id),
  project_id TEXT NOT NULL,
  idempotency_key TEXT NOT NULL,
  ...
  previous_revision_id TEXT REFERENCES cos_execution.authority_side_effect_revisions(revision_id),
  state TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS cos_authority_side_effect_current_idx
  ON cos_execution.authority_side_effect_revisions(operation_id, revision DESC);
CREATE INDEX IF NOT EXISTS cos_authority_side_effect_project_state_idx
  ON cos_execution.authority_side_effect_revisions(project_id, state, recorded_at DESC);
`;

/**
 * Canonical append-only PostgreSQL/Supabase side-effect ledger candidate.
 *
 * Important concurrency rule: inserts use `ON CONFLICT DO NOTHING`; the code
 * never catches a uniqueness exception and then continues querying inside an
 * aborted Postgres transaction. Conflict classification happens through
 * deterministic reads while the transaction advisory lock is held.
 */
export class AuthoritySideEffectPostgresStore implements IAuthoritySideEffectStore {
  constructor(private readonly db: PostgresExecutor) {}

  async ensureSchema(): Promise<void> {
    await this.db.query(AUTHORITY_SIDE_EFFECT_POSTGRES_DDL);
  }
  ...
  async append(
    ...
    return this.db.transaction(async tx => {
      ...
      const transitionDuplicate = await this.selectTransition(
        tx,
        revision.projectId,
        revision.transitionKey,
      );
      if (transitionDuplicate) {
        return classifyTransitionDuplicate(transitionDuplicate, revision);
      }

      await tx.query<SideEffectOperationRow>(`
        INSERT INTO cos_execution.authority_side_effect_operations (
      ...
      const operation = await this.selectOperationForUpdate(
        tx,
        revision.projectId,
        revision.idempotencyKey,
      );
      ...
      validateAppend(revision, current, expectedCurrentRevision);

      const inserted = await tx.query<AuthoritySideEffectRevisionRow>(`
        INSERT INTO cos_execution.authority_side_effect_revisions (
      ...
      const duplicate = await this.selectTransition(
        tx,
        revision.projectId,
        revision.transitionKey,
      );
      if (duplicate) return classifyTransitionDuplicate(duplicate, revision);

      const revisionIdCollision = await this.selectRevisionById(tx, revision.revisionId);
      if (revisionIdCollision) {
        if (revisionIdCollision.contentHash === revision.contentHash) {
          return { revision: revisionIdCollision, appended: false };
        }
        throw new Error(`SIDE_EFFECT_REVISION_ID_COLLISION id=${revision.revisionId}`);
      }

      throw new Error(
        `SIDE_EFFECT_APPEND_CONFLICT operation=${revision.operationId} revision=${revision.revision}`,
      );
    });
  }
  ...
  async getHistory(operationId: string): Promise<AuthoritySideEffectRevision[]> {
    const result = await this.db.query<AuthoritySideEffectRevisionRow>(`
      SELECT * FROM cos_execution.authority_side_effect_revisions
  ...
  async getByIdempotencyKey(
    ...
    const result = await this.db.query<AuthoritySideEffectRevisionRow>(`
      SELECT r.* FROM cos_execution.authority_side_effect_operations o
      JOIN LATERAL (
        SELECT * FROM cos_execution.authority_side_effect_revisions r
  ...
  private async selectOperationForUpdate(
    queryable: PostgresTransaction,
    projectId: string,
    idempotencyKey: string,
  ): Promise<SideEffectOperationRow | null> {
    const result = await queryable.query<SideEffectOperationRow>(`
      SELECT * FROM cos_execution.authority_side_effect_operations
      WHERE project_id=$1 AND idempotency_key=$2
      FOR UPDATE
    `, [projectId, idempotencyKey]);
    return result.rowCount ? structuredClone(result.rows[0]) : null;
  }
  ...
  private async selectCurrent(
    ...
    const result = await queryable.query<AuthoritySideEffectRevisionRow>(`
      SELECT * FROM cos_execution.authority_side_effect_revisions
      WHERE operation_id=$1
...
function classifyTransitionDuplicate(
  existing: AuthoritySideEffectRevision,
  incoming: AuthoritySideEffectRevision,
): AuthoritySideEffectAppendResult {
  if (existing.operationId !== incoming.operationId
    || existing.transitionHash !== incoming.transitionHash) {
    throw new Error(`SIDE_EFFECT_TRANSITION_KEY_CONFLICT key=${incoming.transitionKey}`);
  }
...
function validateAppend(
  incoming: AuthoritySideEffectRevision,
  current: AuthoritySideEffectRevision | null,
  expectedCurrentRevision: number,
): void {
  const currentRevision = current?.revision ?? 0;
  if (currentRevision !== expectedCurrentRevision) {
    throw new Error(
      `STALE_SIDE_EFFECT_REVISION expected=${expectedCurrentRevision} current=${currentRevision}`,
    );
  }
  if (incoming.revision !== currentRevision + 1) {
    throw new Error(
      `SIDE_EFFECT_REVISION_SEQUENCE expected=${currentRevision + 1} incoming=${incoming.revision}`,
    );
  }
  if (!current) {
    if (incoming.revision !== 1
      || incoming.previousRevisionId !== null
      || incoming.state !== 'claimed') {
      throw new Error(`SIDE_EFFECT_INVALID_INITIAL_REVISION operation=${incoming.operationId}`);
    }
    return;
  }
  if (incoming.previousRevisionId !== current.revisionId) {
    throw new Error(`SIDE_EFFECT_REVISION_PARENT_MISMATCH operation=${incoming.operationId}`);
  }
  if (incoming.logicalHash !== current.logicalHash) {
    throw new Error(`SIDE_EFFECT_LOGICAL_IDENTITY_DRIFT operation=${incoming.operationId}`);
  }
  if (Date.parse(incoming.recordedAt) <= Date.parse(current.recordedAt)) {
    throw new Error(`SIDE_EFFECT_SYSTEM_TIME_NOT_MONOTONIC operation=${incoming.operationId}`);
  }
}
...
function rowToRevision(row: AuthoritySideEffectRevisionRow): AuthoritySideEffectRevision {
...
function cloneAndVerify(raw: AuthoritySideEffectRevision): AuthoritySideEffectRevision {
  const revision = structuredClone(raw);
  canonicalSerialize(revision);
  const { contentHash: _ignored, ...payload } = revision;
  const expectedContentHash = canonicalHash128(payload);
  if (expectedContentHash !== revision.contentHash) {
