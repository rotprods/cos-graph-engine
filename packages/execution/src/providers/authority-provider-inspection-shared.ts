...
export interface AuthorityProviderReadTarget {
  ...
  targetKey: string;
  /** Minimum elapsed time between independent authoritative absence observations. */
...
/**
 * Test/reference store only. Production authority must inject a durable store if
 * it intends to promote a repeated absence to `not_applied` across restarts.
 */
export class InMemoryAuthorityAbsenceObservationStore implements AuthorityAbsenceObservationStore {
  ...
  async append(observation: AuthorityAbsenceObservation): Promise<void> {
    ...
    this.records.set(value.observationId, value);
  }
...
export class AuthorityRepeatedAbsenceGate {
  ...
  async observe(input: {
    ...
    const operationContentHash = nonEmpty(
      input.request.operationContentHash,
      'operationContentHash',
    );
    const evidence = canonicalClone(input.evidence, 'absence provider evidence');
    const evidenceHash = canonicalHash128(evidence);
    const identity = {
      provider,
      targetKey,
      operationId: nonEmpty(input.request.operationId, 'operationId'),
      providerIdempotencyKey: nonEmpty(
        input.request.providerIdempotencyKey,
    ...
    const observation: AuthorityAbsenceObservation = {
      ...
      ...content,
      contentHash: canonicalHash128(content),
    };
    ...
    return {
      ...
      evidence: {
        ...proof,
        absenceProofHash: canonicalHash128(proof),
      },
    };
  }
}
 ...
 */
export async function classifyAuthorityProviderRead(
  input: {
    request: AuthorityProviderInspectionRequest;
  ...
  if (exact.length === 1) {
    ...
    if (candidate.operationContentHash !== undefined
      && candidate.operationContentHash !== request.operationContentHash) {
      return {
        status: 'unknown',
    ...
    return {
      ...
      evidence: {
        ...commonEvidence,
        selectedProviderResourceId: candidate.providerResourceId,
      },
    };
  }
  ...
  if (observation.partial) {
    return {
      ...
      compensationCapability: nonEmpty(
        observation.partial.compensationCapability,
      ...
      compensationInput: canonicalClone(
        observation.partial.compensationInput,
        'compensationInput',
      ),
      evidence: {
        ...commonEvidence,
        partialEvidence: observation.partial.evidence,
      },
    };
  }
...
function normalizeReadObservation(
  input: AuthorityProviderReadObservation,
...
function normalizeObservation(input: AuthorityAbsenceObservation): AuthorityAbsenceObservation {
  ...
  const content = {
    ...
    evidenceHash: nonEmpty(copy.evidenceHash, 'evidenceHash'),
  };
  ...
  const expectedHash = canonicalHash128(content);
...
function distinctByObservedAt(
  records: AuthorityAbsenceObservation[],
): AuthorityAbsenceObservation[] {
...
function normalizeRevision(value: string | number | null | undefined): string | number | null {
  ...
  if (typeof value === 'number') {
    if (!Number.isSafeInteger(value) || value < 0) throw new Error('providerRevision number must be a non-negative safe integer');
    return value;
  }
