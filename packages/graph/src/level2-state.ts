...
export class StateMachine {
  ...
  private buildAdjacency(): void {
    this.adj.clear();
    for (const s of this.definition.states) this.adj.set(s.id, []);
    for (const t of this.definition.transitions) {
      if (this.adj.has(t.from)) this.adj.get(t.from)!.push(t);
    }
  }

  addState(config: StateConfig): StateId {
    if (this.definition.states.some(s => s.id === config.id)) throw new Error(`State ${config.id} already exists`);
    this.definition.states.push(config);
    this.buildAdjacency(); return config.id;
  }
  ...
  addTransition(transition: StateTransition): string {
    if (!this.definition.states.some(s => s.id === transition.from)) throw new Error(`From state ${transition.from} not found`);
    if (!this.definition.states.some(s => s.id === transition.to)) throw new Error(`To state ${transition.to} not found`);
  ...
  removeTransition(transitionId: string): void {
    ...
    this.definition.transitions.splice(idx, 1); this.buildAdjacency();
  }

  get id(): EntityId { return this.definition.id; }
  get state(): StateId { return this.context.currentState; }
  get contextData(): StateContext { return { ...this.context }; }
  get states(): StateConfig[] { return [...this.definition.states]; }
  get transitions(): StateTransition[] { return [...this.definition.transitions]; }
  ...
  async send(event: string, payload?: Record<string, unknown>): Promise<boolean> {
    ...
    if (timeout) { clearTimeout(timeout); this.timeouts.delete(this.context.currentState); }
    ...
    for (const listener of this.listeners) { listener(this.context.previousState, this.context.currentState, event); }
    ...
    if (newStateConfig?.entry) { await newStateConfig.entry(this.context); }
  ...
  validate(): string[] {
    const errors: string[] = [];
    if (!this.definition.initial) errors.push('No initial state defined');
    if (this.definition.states.length === 0) errors.push('No states defined');
    ...
    for (const t of this.definition.transitions) {
      if (!this.definition.states.some(s => s.id === t.from)) errors.push(`Dangling transition from: ${t.from}`);
      if (!this.definition.states.some(s => s.id === t.to)) errors.push(`Dangling transition to: ${t.to}`);
    }
    return errors;
  }
  ...
  toJSON(): StateMachineDefinition { return JSON.parse(JSON.stringify(this.definition)); }
  ...
  toMermaid(): string {
    ...
    for (const s of this.definition.states) {
      ...
      if (s.type === 'initial') m += `  [*] --> ${s.id}\n`;
      if (s.type === 'final') m += `  ${s.id} --> [*]\n`;
    }
    for (const t of this.definition.transitions) {
...
export class StateMachineRegistry {
  ...
  remove(id: EntityId): void { this.machines.delete(id); }
