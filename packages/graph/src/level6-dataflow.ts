...
export class DataFlowGraph {
  ...
  addNode(n: DataFlowNode): string {
    if (this.nodes.some(x => x.id === n.id)) throw new Error(`Duplicate DataFlow node ID: ${n.id}`);
  ...
  getNode(nodeId: string): DataFlowNode | undefined { return this.nodes.find(n => n.id === nodeId); }
  getEdge(edgeId: string): DataFlowEdge | undefined { return this.edges.find(e => e.id === edgeId); }
  ...
  buildMLPipeline() {
    ...
    this.addEdge({ id: generateId(), source: 'img', target: 'resize', dataType: 'image', shape: 'HxWx3', sizeBytes: 900000 });
    this.addEdge({ id: generateId(), source: 'resize', target: 'norm', dataType: 'tensor', shape: '224x224x3', sizeBytes: 602112 });
    this.addEdge({ id: generateId(), source: 'norm', target: 'cnn', dataType: 'tensor', shape: '224x224x3' });
    this.addEdge({ id: generateId(), source: 'cnn', target: 'fc', dataType: 'tensor', shape: '7x7x2048' });
    this.addEdge({ id: generateId(), source: 'fc', target: 'out', dataType: 'tensor', shape: '1000' });
  }
  ...
  buildETLPipeline() {
    ...
    this.addEdge({ id: generateId(), source: 'kafka', target: 'parse', dataType: 'bytes', sizeBytes: 1024 });
    this.addEdge({ id: generateId(), source: 'parse', target: 'filter', dataType: 'json' });
    this.addEdge({ id: generateId(), source: 'filter', target: 'enrich', dataType: 'json' });
    this.addEdge({ id: generateId(), source: 'enrich', target: 's3', dataType: 'parquet' });
    this.addEdge({ id: generateId(), source: 'enrich', target: 'dashboard', dataType: 'aggregate' });
  }
  ...
  findBottlenecks(thresholdPercentile: number = 0.8): DataFlowNode[] {
    ...
    if (latencies.length === 0 && throughputs.length === 0) return [];
    ...
    return this.nodes.filter(n =>
      (n.latency && n.latency >= latencyThreshold) || (n.throughput && n.throughput <= throughputThreshold)
    ).slice(0, Math.max(1, Math.floor(this.nodes.length * 0.3)));
  }
  ...
  criticalPath(): DataFlowNode[] {
    ...
    for (const n of this.nodes) { dist.set(n.id, n.latency || 0); prev.set(n.id, null); }
    while (q.length) {
      const u = q.shift()!;
      for (const e of this.edges.filter(e => e.source === u)) {
        const nd = (dist.get(u) || 0) + (this.nodes.find(n => n.id === e.target)?.latency || 0);
        if (nd > (dist.get(e.target) || 0)) { dist.set(e.target, nd); prev.set(e.target, u); }
        const d = inDeg.get(e.target) || 1; inDeg.set(e.target, d - 1);
        if (d - 1 === 0) q.push(e.target);
      }
    }
    let maxNode = ''; let maxDist = 0;
    for (const [id, d] of dist) { if (d > maxDist) { maxDist = d; maxNode = id; } }
    const path: DataFlowNode[] = []; let cur = maxNode;
    while (cur) { const node = this.nodes.find(n => n.id === cur); if (node) path.unshift(node); cur = prev.get(cur) || ''; }
    return path;
  }

  totalLatency(): number { return this.criticalPath().reduce((s, n) => s + (n.latency || 0), 0); }
  ...
  toMermaid(): string {
    ...
    for (const e of this.edges) m += `    ${e.source} -->|"${e.dataType}"| ${e.target}\n`;
    return m;
  }
  ...
  static fromJSON(data: { nodes: DataFlowNode[]; edges: DataFlowEdge[] }): DataFlowGraph {
    const g = new DataFlowGraph(); g.nodes = data.nodes; g.edges = data.edges; g.buildAdjacency(); return g;
  }
}