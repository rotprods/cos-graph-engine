...
export interface WorkflowNode {
  id: EntityId; name: string; type: WorkflowNodeType;
  service?: string; config?: Record<string, unknown>;
  retries?: number; timeout?: number; createdAt: Timestamp;
}

export interface WorkflowEdge {
  id: EntityId; source: EntityId; target: EntityId;
  type: WorkflowEdgeType; condition?: string; createdAt: Timestamp;
}

export interface WorkflowGraph {
  id: EntityId; name: string; description?: string;
  enabled: boolean; nodes: WorkflowNode[]; edges: WorkflowEdge[];
  createdAt: Timestamp;
}
...
export class WorkflowGraphEngine {
  ...
  constructor(name: string, description?: string) {
    this.graph = { id: generateId(), name, description, enabled: true, nodes: [], edges: [], createdAt: new Date().toISOString() };
  }
  ...
  getNode(nodeId: EntityId): WorkflowNode | undefined { return this.graph.nodes.find(n => n.id === nodeId); }
  getNodes(): WorkflowNode[] { return this.graph.nodes; }
  getEdges(): WorkflowEdge[] { return this.graph.edges; }

  getEnabled(): boolean { return this.graph.enabled; }
  setEnabled(enabled: boolean): void { this.graph.enabled = enabled; }
  ...
  topologicalSort(): EntityId[] {
    this.buildAdjacency();
    const inDeg = new Map<EntityId, number>();
    for (const n of this.graph.nodes) inDeg.set(n.id, 0);
    for (const e of this.graph.edges) inDeg.set(e.target, (inDeg.get(e.target) || 0) + 1);
    const q = [...this.graph.nodes.filter(n => inDeg.get(n.id) === 0).map(n => n.id)];
    const r: EntityId[] = [];
    while (q.length > 0) {
      const n = q.shift()!; r.push(n);
  ...
  execute(initialData: Record<string, unknown> = {}): WorkflowNode[] {
    ...
    const order = this.topologicalSort();
    const executed: WorkflowNode[] = [];
    for (const id of order) {
      const node = this.graph.nodes.find(n => n.id === id);
      if (node) executed.push(node);
    }
    return executed;
  }
  ...
  validate(): string[] {
    const errors: string[] = [];
    if (this.detectCycle() !== null) errors.push('Workflow contains a cycle');
    for (const e of this.graph.edges) {
    ...
    if (!this.graph.nodes.some(n => n.type === 'trigger' || n.type === 'webhook')) errors.push('No trigger or webhook node found');
    return errors;
  }
  ...
  toJSON(): WorkflowGraph { return JSON.parse(JSON.stringify(this.graph)); }
  static fromJSON(data: WorkflowGraph): WorkflowGraphEngine { const g = new WorkflowGraphEngine(data.name, data.description); g.graph = data; g.buildAdjacency(); return g; }
