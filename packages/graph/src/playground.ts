...
import { VisualGraphEngine } from './level0-visual';
...
export interface TutorialStep {
  ...
  hint: string;
  validate: (input: string, ctx: PlaygroundContext) => { passed: boolean; message: string };
}
...
const LEVEL_METADATA: Array<{ level: number; name: string; description: string; engine: string }> = [
...
export class LevelPlayground {
  ...
  constructor(level: number) {
    ...
    if (!meta) throw new Error(`Level ${level} not found`);
  ...
  private buildCommands(): PlaygroundCommand[] {
    return [
      {
        name: 'help',
      ...
      },
      {
        ...
        handler: (args) => {
          ...
          (this.context.graph as any).nodes = nodes;
      ...
      },
      {
        ...
        handler: (args) => {
          const id = args[0];
          if (!id) return 'Usage: remove <id>';
          const nodes = (this.context.graph as any).nodes || [];
          const idx = nodes.findIndex((n: any) => n.id === id);
          if (idx < 0) return `Node "${id}" not found`;
          nodes.splice(idx, 1);
      ...
      },
      {
        ...
        handler: (args) => {
          ...
          if (!source || !target) return 'Usage: edge <source> <target> [label]';
          const edges = (this.context.graph as any).edges || [];
          edges.push({ source, target, label: label || '' });
          (this.context.graph as any).edges = edges;
      ...
      },
      {
        ...
        handler: () => {
          ...
          return nodes.map((n: any) => `  ${n.id} (${n.type})`).join('\n');
        },
      },
      {
        name: 'stats',
      ...
      },
      {
        ...
        handler: () => {
          ...
          return nodes.map((n: any) => `[${n.id}]`).join(' -> ') || '(empty)';
        },
      },
      {
        name: 'exit',
...
export class PlaygroundSession {
  private playgrounds: Map<number, LevelPlayground> = new Map();
  ...
  start(level: number): PlaygroundResult {
    if (!this.playgrounds.has(level)) {
      try {
        ...
        return { success: false, output: `Level ${level} not available: ${(e as Error).message}` };
      }
    }
...
export class TutorialRegistry {
  ...
  private registerTutorial(t: Tutorial): void {
  ...
  get(idOrLevel: string): Tutorial | undefined {
    // Try by id
    if (this.tutorials.has(idOrLevel)) return this.tutorials.get(idOrLevel);
    // Try by level number
   ...
   */
  list(): Array<{ id: string; level: number; title: string; steps: number }> {
   ...
   * Contar tutoriales.
   */
  count(): number { return this.tutorials.size; }
...
class TutorialBuilders {
  ...
  static L18_Biological(): Tutorial {
...
export class TutorialRunner {
  ...
  private playground: PlaygroundSession;
   ...
   */
  getCurrent(): Tutorial | null { return this.currentTutorial; }

  /**
   * Obtener el registry.
   */
  getRegistry(): TutorialRegistry { return this.registry; }
}