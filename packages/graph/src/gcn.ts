...
import { EmbeddingGraph, EmbeddingNode } from './level10-embedding';
...
export class GCNLayer {
  ...
  constructor(inputDim: number, outputDim: number, activation: 'relu' | 'tanh' | 'sigmoid' = 'relu') {
  ...
  getWeights(): number[] { return this.weights; }
  getBias(): number { return this.bias; }
}
...
export class GCN {
  ...
  classifyNodesKG(kg: KnowledgeGraphEngine): NodeClassificationResult[] {
    const entities = (kg as any).entities || [];
    if (entities.length === 0) return [];

    // Build features: [type_index, in_degree, out_degree, has_relations]
    const features = entities.map((e: any) => {
      const relations = (kg as any).relations || [];
      const inDegree = relations.filter((r: any) => r.target === e.id).length;
      const outDegree = relations.filter((r: any) => r.source === e.id).length;
      return [
    ...
    for (const r of relations) {
      ...
      if (si >= 0 && ti >= 0) { adj[si][ti] = 1; adj[ti][si] = 1; }
    }
    ...
    const results: NodeClassificationResult[] = [];
  ...
  classifyNodesEmbedding(eg: EmbeddingGraph): NodeClassificationResult[] {
    ...
    const features = nodes.map(n => {
      ...
      for (let i = 0; i < Math.min(vec.length, 5); i++) padded[i] = vec[i];
...
export class GCNPipeline {
   ...
   */
  runGraphClassification(
    nodes: number, edges: number, avgDegree: number, density: number, avgFeature: number,
  ): GraphClassificationResult {
