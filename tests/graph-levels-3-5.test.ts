...
async function testDependencyGraph() {
  ...
  const gid = resolver.createGraph('Test Project', [
    { id: 'react' as EntityId, name: 'react', type: 'library', version: '18.2.0' },
    { id: 'next' as EntityId, name: 'next', type: 'package', version: '14.0.0' },
    { id: 'app' as EntityId, name: 'my-app', type: 'package' },
    { id: 'lodash' as EntityId, name: 'lodash', type: 'library', version: '4.17.21' },
    { id: 'typescript' as EntityId, name: 'typescript', type: 'library', version: '5.3.0' },
  ], [
    { source: 'next' as EntityId, target: 'react' as EntityId, type: 'depends_on', semver: '^18.0.0' },
    { source: 'app' as EntityId, target: 'next' as EntityId, type: 'depends_on', semver: '^14.0.0' },
    { source: 'app' as EntityId, target: 'lodash' as EntityId, type: 'depends_on', semver: '^4.0.0' },
    { source: 'app' as EntityId, target: 'typescript' as EntityId, type: 'depends_on', semver: '^5.0.0' },
    { source: 'next' as EntityId, target: 'typescript' as EntityId, type: 'depends_on', semver: '^5.0.0' },
  ]);
  ...
  assert(firstThree.includes('react' as EntityId) && firstThree.includes('typescript' as EntityId) && firstThree.includes('lodash' as EntityId), 'Topological sort: roots first (react, lodash, typescript)');

  // Ensure app is last
  assert(order[order.length - 1] === 'app' as EntityId, 'Topological sort: app is last (depends on everything)');
  ...
  assert(depth.get('react' as EntityId) === 0, 'Root node has depth 0');
  assert(depth.get('next' as EntityId) === 1, 'Next depends on react → depth 1');
  assert(depth.get('app' as EntityId)! >= 2, 'App depends on next → depth >= 2');
