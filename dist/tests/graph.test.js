...
async function testExecutionGraph() {
    ...
    assert(seqResults.get('s3')?.output && seqResults.get('s3').output.step === 3, 'Sequential: data flows through pipeline');
    // Parallel execution
    const parId = await engine.createGraph('Parallel', [
        { id: 'root', name: 'Root', type: 'function', fn: async (i) => ({ ...i, root: true }) },
        { id: 'brA', name: 'Branch A', type: 'function', fn: async (i) => ({ ...i, branch: 'A' }) },
        { id: 'brB', name: 'Branch B', type: 'function', fn: async (i) => ({ ...i, branch: 'B' }) },
        { id: 'merge', name: 'Merge', type: 'function', fn: async (i) => ({ ...i, merged: true }) },
    ], [
    ...
    assert(sleepResults.get('sleep1')?.output?.slept === 10, 'Sleep node outputs correct ms');
    // Empty graph
