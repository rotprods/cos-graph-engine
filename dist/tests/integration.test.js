...
const index_ts_1 = require("../packages/api/src/index.ts");
...
async function testFullSystem() {
    const server = new index_ts_1.COSServer({ port: 0, logLevel: 'silent' });
    // 1. Register a cell
    const cell = new (class extends index_ts_2.BaseCell {
        constructor() {
            super({
                id: 'cos:int:test', name: 'integration-test', purpose: 'Integration test',
    ...
    // 5. Knowledge Graph
    await server.knowledge.addStatement({ subject: 'COS', predicate: 'tested', object: 'true', confidence: 1, source: 'int', metadata: {}, embedding: undefined });
    ...
    await server.embeddings.store('int-concept', vec, 'concept');
    ...
    // 8. Policy
    await server.policies.addRule({ id: 'pol:int', name: 'int-allow', description: '', effect: 'allow', actions: ['*'], resources: ['*'], conditions: [], priority: 0, enabled: true });
    ...
    // 9. Self-Improvement
    for (let i = 0; i < 3; i++)
        await server.selfImprovement.recordOutput({ q: 'int-' + i }, { r: 'res-' + i });
