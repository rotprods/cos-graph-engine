...
// === addNode duplicate ===
try {
    df.addNode({ id: 'source', name: 'Dup', type: 'source' });
    assert(false, 'L6: Should reject duplicate');
}
catch (e) {
    assert(true, 'L6: Rejects duplicate node id');
}
...
try {
    df.removeNode('nonexistent');
    assert(false, 'L6: Should reject remove nonexistent');
}
catch (e) {
    assert(true, 'L6: Rejects remove nonexistent node');
}
...
const bad = new level6_dataflow_1.DataFlowGraph();
bad.addNode({ id: 'a', name: 'A', type: 'source' });
try {
    bad.addEdge({ id: 'x', source: 'a', target: 'nonexistent' });
    assert(false, 'L6: Should reject');
}
catch (e) {
    assert(true, 'L6: Rejects dangling edge at addEdge');
}
...
const restored = level6_dataflow_1.DataFlowGraph.fromJSON(saved);
...
// === Empty graph ===
const empty = new level6_dataflow_1.DataFlowGraph();
...
// === Pipeline with cycle ===
const cyclic = new level6_dataflow_1.DataFlowGraph();
...
if (f > 0)
    process.exit(1);
//# sourceMappingURL=test-level6-dataflow.js.map