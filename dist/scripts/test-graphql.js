...
const graphql_1 = require("../packages/graph/src/graphql");
let passed = 0;
let failed = 0;
...
async function main() {
    ...
    assert(typeof graphql_1.GQL_SCHEMA === 'string', 'Schema is a string');
    assert(graphql_1.GQL_SCHEMA.length > 500, 'Schema has substantial content');
    assert(graphql_1.GQL_SCHEMA.includes('type Query'), 'Schema has Query type');
    assert(graphql_1.GQL_SCHEMA.includes('type Mutation'), 'Schema has Mutation type');
    assert(graphql_1.GQL_SCHEMA.includes('type Graph'), 'Schema has Graph type');
    assert(graphql_1.GQL_SCHEMA.includes('type Node'), 'Schema has Node type');
    assert(graphql_1.GQL_SCHEMA.includes('type Edge'), 'Schema has Edge type');
    assert(graphql_1.GQL_SCHEMA.includes('type PageInfo'), 'Schema has PageInfo type');
    assert(graphql_1.GQL_SCHEMA.includes('type MutationResult'), 'Schema has MutationResult type');
    assert(graphql_1.GQL_SCHEMA.includes('type BatchResult'), 'Schema has BatchResult type');
    assert(graphql_1.GQL_SCHEMA.includes('type GraphStats'), 'Schema has GraphStats type');
    assert(graphql_1.GQL_SCHEMA.includes('type Level'), 'Schema has Level type');
    assert(graphql_1.GQL_SCHEMA.includes('type ServiceHealth'), 'Schema has ServiceHealth type');
    assert(graphql_1.GQL_SCHEMA.includes('enum EdgeDirection'), 'Schema has EdgeDirection enum');
    section('Schema — Queries');
    assert(graphql_1.GQL_SCHEMA.includes('graph(level: Int!, id: ID!)'), 'Schema has graph query');
    assert(graphql_1.GQL_SCHEMA.includes('search(level: Int!, query: String!'), 'Schema has search query');
    assert(graphql_1.GQL_SCHEMA.includes('node(level: Int!, id: ID!)'), 'Schema has node query');
    assert(graphql_1.GQL_SCHEMA.includes('graphs(level: Int!'), 'Schema has graphs query');
    assert(graphql_1.GQL_SCHEMA.includes('graphStats(level: Int!)'), 'Schema has graphStats query');
    assert(graphql_1.GQL_SCHEMA.includes('levels'), 'Schema has levels query');
    assert(graphql_1.GQL_SCHEMA.includes('health'), 'Schema has health query');
    section('Schema — Mutations');
    assert(graphql_1.GQL_SCHEMA.includes('addNode(level: Int!'), 'Schema has addNode mutation');
    assert(graphql_1.GQL_SCHEMA.includes('addEdge(level: Int!'), 'Schema has addEdge mutation');
    assert(graphql_1.GQL_SCHEMA.includes('removeNode(level: Int!'), 'Schema has removeNode mutation');
    assert(graphql_1.GQL_SCHEMA.includes('removeEdge(level: Int!'), 'Schema has removeEdge mutation');
    assert(graphql_1.GQL_SCHEMA.includes('createGraph(level: Int!'), 'Schema has createGraph mutation');
    assert(graphql_1.GQL_SCHEMA.includes('deleteGraph(level: Int!'), 'Schema has deleteGraph mutation');
    assert(graphql_1.GQL_SCHEMA.includes('batchAddNodes'), 'Schema has batchAddNodes mutation');
    assert(graphql_1.GQL_SCHEMA.includes('batchAddEdges'), 'Schema has batchAddEdges mutation');
    assert(graphql_1.GQL_SCHEMA.includes('clearGraph'), 'Schema has clearGraph mutation');
    // =============================================
    // T-14.2: Resolvers
    // =============================================
    section('GQLEngine — Schema Access');
    const engine = new graphql_1.GQLEngine();
    const schema = engine.getSchema();
    assert(schema === graphql_1.GQL_SCHEMA, 'getSchema returns schema');
    ...
    assert(createViaExec.createGraph.success === true, 'Execute createGraph works');
    const graphId = createViaExec.createGraph.graph.id;
    const addViaExec = engine.execute('addNode', { level: 7, graphId, input: { id: 'x1', label: 'Input' } });
    assert(addViaExec.addNode.success === true, 'Execute addNode works');
    const healthViaExec = engine.execute('health', {});
    assert(healthViaExec.health.status === 'healthy', 'Execute health works');
    const levelsViaExec = engine.execute('levels', {});
    assert(Array.isArray(levelsViaExec.levels), 'Execute levels returns array');
    const statsViaExec = engine.execute('graphStats', { level: 7 });
    assert(statsViaExec.graphStats.totalGraphs >= 1, 'Execute graphStats works');
    // Unknown operation
    const unknownOp = engine.execute('unknownOp', {});
    assert(unknownOp.error !== undefined, 'Unknown operation returns error');
...
//# sourceMappingURL=test-graphql.js.map