...
async function main() {
    ...
    const restored = csr_1.CSRGraph.fromJSON(json);
    ...
    const memGraph = new csr_1.CSRGraph();
    const M = 200;
    for (let i = 0; i < M; i++)
        memGraph.addNode({ id: `m${i}` });
    for (let i = 0; i < M; i++) {
    ...
    const large = new csr_1.CSRGraph();
    const N = 100;
    for (let i = 0; i < N; i++) {
    ...
    const adj = new csr_1.CompressedAdjacency();
    adj.addEdge('a', 'b');
    adj.addEdge('a', 'c');
    adj.addEdge('b', 'd');
    ...
    const adj2 = new csr_1.CompressedAdjacency();
    adj2.addEdge('x', 'y');
    adj2.clear();
...
//# sourceMappingURL=test-csr.js.map