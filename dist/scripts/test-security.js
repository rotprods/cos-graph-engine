...
const security_1 = require("../packages/graph/src/security");
let passed = 0;
let failed = 0;
...
function testSanitizer() {
    ...
    const sanitizer = new security_1.InputSanitizer();
...
function testValidator() {
    ...
    const validator = new security_1.GraphValidator();
...
async function testGuard() {
    ...
    const guard = new security_1.SecurityGuard({ maxOpsPerWindow: 5, rateLimitWindowMs: 10_000, maxNodes: 10, maxEdges: 20, defaultTimeoutMs: 500, maxRecursionDepth: 10 });
...
function testLevelSecurity() {
    ...
    const levelSec = new security_1.LevelSecurity();
