...
const index_ts_1 = require("../packages/graph/src/index.ts");
const fs = __importStar(require("fs"));
const results = [];
let passed = 0, failed = 0;
function record(name, ops, timeMs, errs) {
    const r = {
...
async function benchmarkExecutionGraph() {
    ...
    const largeEdges = [];
    for (let i = 0; i < 50; i++) {
        largeNodes.push({ id: `n${i}`, name: `N${i}`, type: 'function', fn: async (i) => i });
        if (i > 0)
            largeEdges.push({ source: `n${i - 1}`, target: `n${i}` });
    }
...
async function benchmarkStateGraph() {
    const registry = new index_ts_1.StateMachineRegistry();
    ...
    // Large FSM
    const manyStates = [];
    const manyTransitions = [];
    for (let i = 0; i < 50; i++) {
        manyStates.push({ id: `s${i}`, label: `State ${i}`, type: (i === 0 ? 'initial' : i === 49 ? 'final' : 'normal') });
        if (i > 0)
            manyTransitions.push({ from: `s${i - 1}`, to: `s${i}`, event: `next_${i}` });
    }
    const lgStart = Date.now();
    for (let i = 0; i < 20; i++) {
        const m = registry.create('Big', manyStates, manyTransitions, 's0');
        for (let j = 1; j < 50; j++)
            await m.send(`next_${j}`);
    }
...
//# sourceMappingURL=graph-benchmark.js.map