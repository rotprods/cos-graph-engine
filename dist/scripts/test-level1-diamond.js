...
async function main() {
    ...
    const engine = new index_1.ExecutionGraphEngine();
    ...
    const diamondId = await engine.createGraph('Diamond', [
        { id: 'A', name: 'Source A', type: 'function', fn: async (i) => ({ ...i, from: 'A', val: 1 }) },
        { id: 'B', name: 'Source B', type: 'function', fn: async (i) => ({ ...i, from: 'B', val: 2 }) },
        { id: 'C', name: 'Merge C', type: 'function', fn: async (i) => ({ ...i, merged: 'C' }) },
        { id: 'D', name: 'Merge D', type: 'function', fn: async (i) => ({ ...i, merged: 'D' }) },
    ], [
        { source: 'A', target: 'C' },
        { source: 'A', target: 'D' },
        { source: 'B', target: 'C' },
    ...
    const chainId = await engine.createGraph('Chain', [
        { id: 's1', name: 'Step 1', type: 'function', fn: async (i) => ({ ...i, step: 1 }) },
        { id: 's2', name: 'Step 2', type: 'function', fn: async (i) => ({ ...i, step: 2 }) },
        { id: 's3', name: 'Step 3', type: 'function', fn: async (i) => ({ ...i, step: 3 }) },
    ], [
        { source: 's1', target: 's2' },
        { source: 's2', target: 's3' },
    ]);
    ...
    assert(chainResults.get('s3')?.output?.step === 3, 'Chain: data flows through pipeline');
    // Empty graph
    ...
    const singleId = await engine.createGraph('Single', [
        { id: 'only', name: 'Only Node', type: 'function', fn: async (i) => ({ ...i, done: true }) },
    ], []);
    ...
    assert(singleResults.get('only')?.output?.done, 'Single: function executed');
    // Disconnected graph (two independent subgraphs)
    const discId = await engine.createGraph('Disconnected', [
        { id: 'a', name: 'A', type: 'function', fn: async (i) => ({ ...i, letter: 'A' }) },
        { id: 'b', name: 'B', type: 'function', fn: async (i) => ({ ...i, letter: 'B' }) },
        { id: 'c', name: 'C', type: 'function', fn: async (i) => ({ ...i, letter: 'C' }) },
    ], [
        { source: 'a', target: 'b' },
        // c is disconnected
    ]);
...
main();
//# sourceMappingURL=test-level1-diamond.js.map