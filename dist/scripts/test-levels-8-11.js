...
(function testL8() {
    ...
    // duplicate
    try {
        kg.addEntity({ id: 'einstein', name: 'Dup', type: 'concept' });
        assert(false, 'L8: Should reject duplicate');
    }
    catch (e) {
        assert(true, 'L8: Rejects duplicate entity');
    }
    // addRelation
    ...
    // dangling relation
    try {
        kg.addRelation({ id: 'r3', source: 'nonexistent', target: 'physics', type: 'related_to' });
        assert(false, 'L8: Should reject dangling');
    }
    catch (e) {
        assert(true, 'L8: Rejects dangling relation source');
    }
    // removeEntity
    ...
    // buildAIEcosystem
    const kg2 = new level8_knowledge_1.KnowledgeGraphEngine();
    ...
    // buildCOS
    const kg3 = new level8_knowledge_1.KnowledgeGraphEngine();
    kg3.buildCOS();
    ...
    const restored = level8_knowledge_1.KnowledgeGraphEngine.fromJSON(saved);
...
(function testL9() {
    ...
    // buildAnimalTaxonomy
    const sg2 = new level9_semantic_1.SemanticGraph();
    ...
    const restored = level9_semantic_1.SemanticGraph.fromJSON(saved);
...
(function testL10() {
    ...
    // duplicate
    try {
        eg.addNode({ id: 'doc1', source: 'Dup', embedding: [0, 0, 0], metadata: {} });
        assert(false, 'L10: Should reject');
    }
    catch (e) {
        assert(true, 'L10: Rejects duplicate');
    }
    // addEdge
    ...
    // buildAIModelGraph
    const eg2 = new level10_embedding_1.EmbeddingGraph();
    ...
    const restored = level10_embedding_1.EmbeddingGraph.fromJSON(saved);
...
(function testL11() {
    ...
    // duplicate entity
    try {
        rag.addEntity('sky', 'Dup', 'concept');
        assert(false, 'L11: Should reject dup entity');
    }
    catch (e) {
        assert(true, 'L11: Rejects duplicate entity');
    }
    // addRelation
    ...
    // buildDemo
    const rag2 = new level11_graphrag_1.GraphRAGEngine();
    rag2.buildDemo();
    ...
    const restored = level11_graphrag_1.GraphRAGEngine.fromJSON(saved);
...
if (f > 0)
    process.exit(1);
//# sourceMappingURL=test-levels-8-11.js.map