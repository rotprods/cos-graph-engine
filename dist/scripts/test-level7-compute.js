...
async function main() {
    ...
    // ============ TEST 1: Empty Graph ============
    const empty = new level7_compute_1.ComputationalGraph();
    ...
    const expr = new level7_compute_1.ComputationalGraph();
    ...
    const mlp = new level7_compute_1.ComputationalGraph();
    mlp.buildMLP();
    ...
    assert(mlpGrads.get('logit0') !== undefined && Math.abs(mlpGrads.get('logit0')) > 0, `MLP: logit0 has non-zero gradient (got ${mlpGrads.get('logit0')})`);
    assert(mlpGrads.get('logit1') !== undefined && Math.abs(mlpGrads.get('logit1')) > 0, `MLP: logit1 has non-zero gradient (got ${mlpGrads.get('logit1')})`);
    // Parameters should have non-zero gradients
    assert(mlpGrads.get('w1') !== undefined && Math.abs(mlpGrads.get('w1')) > 0, `MLP: w1 has non-zero gradient (got ${mlpGrads.get('w1')})`);
    assert(mlpGrads.get('w2') !== undefined && Math.abs(mlpGrads.get('w2')) > 0, `MLP: w2 has non-zero gradient (got ${mlpGrads.get('w2')})`);
    // ============ TEST 4: Mermaid ============
    ...
    const restored = level7_compute_1.ComputationalGraph.fromJSON(json);
    ...
    const mlp2 = new level7_compute_1.ComputationalGraph();
    mlp2.buildMLP();
    mlp2.forward({ x: 1 });
...
main();
//# sourceMappingURL=test-level7-compute.js.map