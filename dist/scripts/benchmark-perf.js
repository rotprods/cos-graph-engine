...
async function benchmarkL1Queue() {
    ...
    function oldQueueRefill(nodes, edges, maxConcurrency) {
    ...
    function newQueueRefill(nodes, edges, maxConcurrency) {
        const inDegree = new Map();
        const adj = new Map();
        for (const id of nodes) {
            inDegree.set(id, 0);
    ...
    function buildChain(n) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            nodes.push(id);
            if (i > 0)
                edges.push({ source: `n${i - 1}`, target: id });
        }
        return { nodes, edges };
    }
    ...
    function buildDiamond(n) {
        ...
        for (let i = 0; i < n; i++) {
            for (let j = 1; j <= 2 && i + j < n; j++) {
                edges.push({ source: `n${i}`, target: `n${i + j}` });
            }
        }
        return { nodes, edges };
    }
...
function benchmarkL3Adjacency() {
    ...
    function oldDetectCycle(nodes, edges) {
    ...
    // NEW: adjacency map
    function newDetectCycle(nodes, edges) {
        const adj = new Map();
        for (const id of nodes)
            adj.set(id, []);
        for (const e of edges)
...
main();
//# sourceMappingURL=benchmark-perf.js.map