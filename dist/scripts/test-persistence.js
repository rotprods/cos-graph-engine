...
const sharding_1 = require("../packages/graph/src/sharding");
const cache_1 = require("../packages/graph/src/cache");
const replication_1 = require("../packages/graph/src/replication");
let passed = 0;
let failed = 0;
...
async function main() {
    ...
    const hash = new sharding_1.ConsistentHash(4);
    ...
    const shard = new sharding_1.GraphShard(0, 5);
    ...
    const mgr = new sharding_1.ShardManager({ totalShards: 4, strategy: 'hash' });
    ...
    const rangeMgr = new sharding_1.ShardManager({ totalShards: 4, strategy: 'range' });
    ...
    const levelMgr = new sharding_1.ShardManager({ totalShards: 4, strategy: 'level' });
    ...
    const l1cache = new cache_1.L1Cache(100);
    ...
    const smallL1 = new cache_1.L1Cache(3);
    smallL1.set('a', 1);
    smallL1.set('b', 2);
    smallL1.set('c', 3);
    ...
    const l2cache = new cache_1.L2Cache(500);
    ...
    const shortTtlL2 = new cache_1.L2Cache(1); // 1ms TTL
    ...
    const mlc = new cache_1.MultiLevelCache(100, 60000);
    assert(mlc !== undefined, 'MultiLevelCache constructed');
    assert(mlc.l1 instanceof cache_1.L1Cache, 'Has L1');
    assert(mlc.l2 instanceof cache_1.L2Cache, 'Has L2');
    assert(mlc.l3 instanceof cache_1.L3Cache, 'Has L3');
    ...
    const mlc2 = new cache_1.MultiLevelCache(100, 60000);
    mlc2.set('a', 'l1-value');
    ...
    const mlc3 = new cache_1.MultiLevelCache(100, 60000);
    ...
    const mlc4 = new cache_1.MultiLevelCache(100, 60000);
    ...
    const rConfig = { id: 'r1', role: 'master', syncInterval: 1000, conflictStrategy: 'last_write_wins' };
    const rep = new replication_1.Replica(rConfig);
    ...
    assert(rn.value.data === 'hello', 'Node data correct');
    ...
    const rep2 = new replication_1.Replica({ id: 'r2', role: 'slave', syncInterval: 1000, conflictStrategy: 'last_write_wins' });
    ...
    const rep3 = new replication_1.Replica({ id: 'r3', role: 'master', syncInterval: 1000, conflictStrategy: 'last_write_wins' });
    const rep4 = new replication_1.Replica({ id: 'r4', role: 'master', syncInterval: 1000, conflictStrategy: 'last_write_wins' });
    ...
    const ms = new replication_1.MasterSlaveReplication('master-1', 'last_write_wins');
    ...
    assert(slave1Node.value === 'master-data', 'Slave has correct value');
    ...
    assert(readVal.value === 'master-data', 'Read correct value');
    ...
    const mm = new replication_1.MultiMasterReplication('last_write_wins');
