...
const convert_1 = require("../packages/graph/src/convert");
let passed = 0;
let failed = 0;
...
async function main() {
    ...
    const builder = new convert_1.GraphBuilder();
    ...
    const converter = new convert_1.GraphConverter();
    ...
    try {
        converter.parse('content', 'graphml');
        assert(true, 'Parse with valid format works');
    }
    catch (e) {
        assert(false, 'Valid format should not throw');
    }
    try {
        converter.serialize(graph, 'invalid');
        assert(false, 'Should throw on invalid format');
    }
    catch (e) {
        assert(e.message.includes('Unsupported'), 'Invalid format throws Unsupported');
    }
    // =============================================
    // Summary
    // =============================================
...
//# sourceMappingURL=test-convert.js.map