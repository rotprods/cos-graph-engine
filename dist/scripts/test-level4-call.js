...
// === addNode duplicate ===
try {
    builder.addNode(gid, { id: 'root', name: 'dup', type: 'function' });
    assert(false, 'L4: Should reject duplicate');
}
catch (e) {
    assert(true, 'L4: Rejects duplicate node id');
}
...
const restored = level4_call_1.CallGraphBuilder.fromJSON(saved);
...
if (f > 0)
    process.exit(1);
//# sourceMappingURL=test-level4-call.js.map