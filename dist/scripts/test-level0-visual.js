...
// === Creation ===
const g = new level0_visual_1.VisualGraphEngine('Test Flow');
...
// === addNode duplicate ===
try {
    g.addNode({ id: 'custom-id', label: 'Duplicate' });
    assert(false, 'L0: Should reject duplicate id');
}
catch (e) {
    assert(true, 'L0: Rejects duplicate id');
}
...
try {
    g.addEdge('nonexistent', n2, 'bad');
    assert(false, 'L0: Should reject dangling source');
}
catch (e) {
    assert(true, 'L0: Rejects dangling source');
}
// === addEdge dangling target ===
try {
    g.addEdge(n1, 'nonexistent', 'bad');
    assert(false, 'L0: Should reject dangling target');
}
catch (e) {
    assert(true, 'L0: Rejects dangling target');
}
...
try {
    ...
    assert(false, 'L0: Should reject remove nonexistent');
}
catch (e) {
    assert(true, 'L0: Rejects remove nonexistent node');
}
// === Mutation API: removeEdge ===
g.removeEdge(e1);
assert(g['graph'].edges.length === 1, 'L0: removeEdge removes edge');
try {
    g.removeEdge('nonexistent');
    assert(false, 'L0: Should reject remove nonexistent edge');
}
catch (e) {
    assert(true, 'L0: Rejects remove nonexistent edge');
}
// === Renderers ===
const g2 = new level0_visual_1.VisualGraphEngine('Render Test');
g2.buildFlowchart();
...
// === Validation ===
const gv = new level0_visual_1.VisualGraphEngine('Good');
gv.addNode({ label: 'A' });
gv.addNode({ label: 'B' });
...
const gb = new level0_visual_1.VisualGraphEngine('Bad');
gb.addNode({ label: 'X' });
...
const restored = level0_visual_1.VisualGraphEngine.fromJSON(saved);
...
// === createFromEdges ===
const g4 = new level0_visual_1.VisualGraphEngine('Edges');
...
// === Direction ===
const g5 = new level0_visual_1.VisualGraphEngine('LR Flow');
...
// === Edge styles ===
const g6 = new level0_visual_1.VisualGraphEngine('Styles');
...
// === Node types ===
const g7 = new level0_visual_1.VisualGraphEngine('Types');
...
// === Empty graph metrics ===
const empty = new level0_visual_1.VisualGraphEngine('Empty');
...
if (f > 0)
    process.exit(1);
//# sourceMappingURL=test-level0-visual.js.map