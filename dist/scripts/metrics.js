...
const metrics = [];
function measure(name, category, target, actual, unit) {
...
async function main() {
    ...
    for (let i = 0; i < 500; i++)
        await bus.publish({ type: 'metric', source: 'm', payload: {}, severity: 'info', metadata: {} });
    ...
    const ids = [];
    for (let i = 0; i < 200; i++)
        ids.push(await mem.store({ i }, 'cache', { tags: ['m'], importance: 0.3, ttl: 60 }));
    ...
    for (let i = 0; i < 100; i++)
        await kg.addStatement({ subject: `S${i}`, predicate: 'p', object: `O${i}`, confidence: 1, source: 'm', metadata: {}, embedding: undefined });
    ...
    let errCatches = 0;
    try {
        throw new core.CellError('TEST', 'test');
    }
    catch {
        errCatches++;
    }
    try {
        throw new core.ValidationError('test');
    }
    catch {
        errCatches++;
    }
    try {
        throw new core.TimeoutError('test');
    }
    catch {
        errCatches++;
    }
    ...
    const taskIds = [];
    for (let i = 0; i < 20; i++)
        taskIds.push(await scheduler.enqueue({ type: 't', priority: 1, target: 'c', input: {}, context: { traceId: 't' }, dependencies: [], maxRetries: 1, timeout: 5000, policies: [] }));
    scheduler.start();
    ...
    console.log('\n📍 QUALITY METRICS\n');
    // Test coverage
    const testFiles = require('fs').readdirSync('../tests').filter((f) => f.endsWith('.test.ts'));
    ...
    await pol.addRule({ id: 'p:deny', name: 'deny-root', description: '', effect: 'deny', actions: ['*'], resources: ['/admin'], conditions: [], priority: 1, enabled: true });
