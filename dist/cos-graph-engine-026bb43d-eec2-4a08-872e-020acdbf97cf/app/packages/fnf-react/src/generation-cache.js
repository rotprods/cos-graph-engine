...
exports.applyGenerations = applyGenerations;
exports.prependGenerations = prependGenerations;
exports.removeGenerationQueries = removeGenerationQueries;
 ...
 */
function applyGenerations(queryClient, generations, opts) {
    if (generations.length === 0)
        return;
    const byId = new Map(generations.map(g => [g.id, g]));
    for (const g of generations) {
        if (queryClient.getQueryState(keys_1.fnfKeys.job(g.id, opts))) {
            queryClient.setQueryData(keys_1.fnfKeys.job(g.id, opts), (prev) => {
                const folded = (0, generation_fold_1.foldGeneration)(prev, g);
                return folded === prev ? undefined : folded; // undefined bails out — no write, stable reference
            });
        }
        if (g.jobSetId !== undefined && queryClient.getQueryState(keys_1.fnfKeys.jobSet(g.jobSetId, opts))) {
            queryClient.setQueryData(keys_1.fnfKeys.jobSet(g.jobSetId, opts), (prev) => {
                if (!prev)
                    return undefined;
                let changed = false;
                const next = prev.map((member) => {
                    ...
                    if (!fresh)
                        return member;
                    const folded = (0, generation_fold_1.foldGeneration)(member, fresh);
                    if (folded !== member)
                        changed = true;
                    return folded;
                });
    ...
    queryClient.setQueriesData({ queryKey: jobsRootKey(opts) }, (prev) => {
        if (!prev)
            return undefined;
        let changed = false;
        const pages = prev.pages.map((page) => {
            ...
            const items = page.items.map((item) => {
                ...
                if (!fresh)
                    return item;
                const folded = (0, generation_fold_1.foldGeneration)(item, fresh);
                if (folded !== item)
                    pageChanged = true;
                return folded;
            });
 ...
 */
function prependGenerations(queryClient, query, generations, opts) {
    queryClient.setQueryData(keys_1.fnfKeys.jobs(query, opts), (prev) => {
...
function removeGenerationQueries(queryClient, opts) {
    if (opts?.scopeKey) {
        queryClient.removeQueries({ queryKey: [...keys_1.fnfKeys.scope(opts.scopeKey), 'job'] });
        queryClient.removeQueries({ queryKey: [...keys_1.fnfKeys.scope(opts.scopeKey), 'job-set'] });
        queryClient.removeQueries({ queryKey: keys_1.fnfKeys.scopedJobsRoot(opts.scopeKey) });
        queryClient.removeQueries({ queryKey: [...keys_1.fnfKeys.scope(opts.scopeKey), 'cost'] });
        return;
    }
    queryClient.removeQueries({ queryKey: ['fnf', 'job'] });
    queryClient.removeQueries({ queryKey: ['fnf', 'job-set'] });
    queryClient.removeQueries({ queryKey: keys_1.fnfKeys.jobsRoot });
    queryClient.removeQueries({ queryKey: ['fnf', 'cost'] });
}
function jobsRootKey(opts) {
    return opts?.scopeKey ? keys_1.fnfKeys.scopedJobsRoot(opts.scopeKey) : keys_1.fnfKeys.jobsRoot;
}
//# sourceMappingURL=generation-cache.js.map