...
exports.seedance2_0 = exports.Seedance2AspectRatio = void 0;
...
const FRAME_ROLES = ['start_image', 'end_image'];
const IMAGE_ROLES = ['image', 'start_image', 'end_image'];
...
exports.seedance2_0 = (0, define_job_1.defineJob)({
    ...
    params: {
        ...
        media: {
            ...
            rules: [
                ...
                (0, media_1.requiresOneOf)('audio', ['image', 'video', 'start_image', 'end_image']),
                // reference images (incl. frames) are capped at 9 in total
                (0, media_1.maxTotal)(['image', 'start_image', 'end_image'], 9),
                ...
                (0, media_1.dimensionsWithin)(['image', 'start_image', 'end_image'], { minSide: 300, maxSide: 6000, ratio: [0.4, 2.5] }),
                (0, media_1.dimensionsWithin)(['video'], { minSide: 300, maxSide: 6000, minPixels: 409_600, ratio: [0.4, 2.5] }),
                (0, media_1.durationsWithin)(['video'], { each: [2, 15] }),
                (0, media_1.durationsWithin)(['audio'], { each: [2, 15] }),
                // videos + audios share one 15-second budget
                (0, media_1.durationsWithin)(['video', 'audio'], { total: 15 }),
            ],
        },
        settings: {
            ...
            duration: z_1.z.duration({ min: 4, max: 15 }),
            aspectRatio: z_1.z.wire('aspect_ratio', z_1.z.aspectRatio(Object.values(exports.Seedance2AspectRatio))),
            // Per-mode product default applied in finalize: std → 1080p, fast → 720p
            // (gen-panel fields.ts; fast never offers 1080p).
            resolution: z_1.z.optional(z_1.z.enum(['480p', '720p', '1080p'])),
            // Public knob; maps to the wire `model` (seedance_2_0 / seedance_2_0_fast).
            mode: z_1.z._default(z_1.z.enum(['std', 'fast']), 'std'),
            generateAudio: z_1.z.wire('generate_audio', z_1.z._default(z_1.z.boolean(), true)),
            bitrateMode: z_1.z.wire('bitrate_mode', z_1.z._default(z_1.z.enum(['standard', 'high']), 'standard')),
        },
    },
    credits: ({ settings }) => {
    ...
    validate: ({ prompt, media, settings }) => {
        ...
        const visuals = (0, checks_1.countRefs)(media, 'image') + (0, checks_1.countRefs)(media, 'start_image')
            + (0, checks_1.countRefs)(media, 'end_image') + (0, checks_1.countRefs)(media, 'video');
        ...
        if (FRAME_ROLES.some(role => (0, checks_1.countRefs)(media, role) > 0) && settings.aspectRatio !== 'auto')
            issues.push({ loc: ['settings', 'aspectRatio'], msg: 'aspectRatio must be \'auto\' when a start/end frame is attached' });
        issues.push(...(0, checks_1.oneOf)('aspectRatio', settings.aspectRatio, Object.values(exports.Seedance2AspectRatio)));
        // built.ts: !duration || duration < 4 || duration > 15 is a hard reject.
        issues.push(...(0, checks_1.intRange)('duration', settings.duration, 4, 15));
        issues.push(...(0, checks_1.intRange)('batchSize', settings.batchSize, 1, 4));
    ...
    finalize: (wire, input) => {
        ...
        const resolution = (wire.resolution ?? (mode === 'fast' ? '720p' : '1080p'));
        ...
        let ratio = wire.aspect_ratio;
        if (ratio === 'auto') {
            const size = (0, dimensions_1.firstSizeMeta)(input.media, IMAGE_ROLES)
                ?? (typeof wire.width === 'number' && typeof wire.height === 'number' && wire.height > 0
                    ? { width: wire.width, height: wire.height }
                    : undefined);
            if (size)
                ratio = (0, dimensions_1.closestRatioBySize)(CONCRETE_RATIOS, size);
        }
        ...
        const { width, height } = (0, dimensions_1.lookupSize)(SIZE_MAP, resolution, ratio === 'auto' ? '16:9' : ratio);
        return {
            ...wire,
    ...
    restore: (wire) => {
        ...
        const medias = Array.isArray(wire.medias) ? wire.medias : [];
        if (medias.some(m => m?.role === 'start_image' || m?.role === 'end_image'))
            restored.aspect_ratio = 'auto';
        return restored;
    },
});
//# sourceMappingURL=seedance-2-0.js.map