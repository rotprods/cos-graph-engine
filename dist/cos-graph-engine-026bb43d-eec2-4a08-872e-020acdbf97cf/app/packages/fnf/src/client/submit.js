...
exports.generationsFromBody = generationsFromBody;
const errors_1 = require("../errors");
const observability_1 = require("../observability");
const spec_1 = require("../spec");
const context_1 = require("./context");
async function submit(ctx, input) {
    return (0, observability_1.observeAsync)(ctx.observability, 'fnf.job.submit', {
        model: input.model,
        ...
        const settled = await Promise.allSettled(Array.from({ length: count }, () => ctx.adapter.createJobs({
        ...
        if (generations.length === 0) {
            ...
            throw failed[0] ?? new errors_1.UnknownSubmitResponseError(undefined, fulfilled?.value);
        }
...
async function confirmSubmission(ctx, jobSetType, params) {
    ...
    try {
        ...
        throw err instanceof errors_1.ApiJobError ? err : new errors_1.ConfirmationRejectedError(err instanceof Error ? err.message : undefined);
    }
    return typeof token === 'string' ? token : undefined;
}
async function safeSubmit(ctx, input) {
    try {
 ...
 */
function generationsFromBody(body, entry, input) {
    if (Array.isArray(body)) {
        ...
        if (body.length > 0 && body.every(item => isObject(item) && typeof item.id === 'string'))
            return body.map(job => (0, spec_1.parseGeneration)(job, entry));
...
function pendingGeneration(id, entry, input) {
...
function asError(err) {
    return err instanceof errors_1.ApiJobError ? err : new errors_1.ApiJobError('unexpected', err instanceof Error ? err.message : String(err));
}
