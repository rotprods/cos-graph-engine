...
const client = (0, index_1.createJobClient)({
    adapter: backend,
    jobs: [index_1.nanoBanana2, index_1.seedance2_0], // ← the source of autocomplete
});
const imageClient = (0, index_1.createJobClient)({
    adapter: backend,
    jobs: [index_1.soulV2Image, index_1.soulCinemaImage, index_1.gptImage2, index_1.seedreamV4_5, index_1.nanoBanana2, index_1.nanoBananaFlash, index_1.nanoBanana2Upscale, index_1.recraftV41Image, index_1.topazImageUpscale, index_1.topazImageGenerativeUpscale],
});
const videoClient = (0, index_1.createJobClient)({
    adapter: backend,
    jobs: [index_1.seedance2_0, index_1.kling3_0, index_1.kling3MotionControl, index_1.happyHorse, index_1.grokImagine, index_1.grokImagineV15, index_1.veo3_1Lite, index_1.wan27, index_1.topazVideoUpscale, index_1.higgsfieldVideoUpscale, index_1.soraEnhanceVideo, index_1.bytedanceVideoUpscale],
});
const profileClient = (0, index_1.createProfileClient)({
    profileAdapter: profileBackend,
});
const observer = (event) => {
    expectType(event);
    expectType(event.phase);
    expectType(event.id);
    expectType(event.traceId);
    expectType(event.attributes.model);
};
...
void (0, index_1.createObservabilityContext)(observabilityOptions);
void (0, index_1.createNoopObserver)();
// @ts-expect-error observability attributes accept only primitive metadata values
const badObservabilityOptions = { attributes: { unsafe: { nested: true } } };
...
void client.submit({ model: 'seedance_2_0', settings: { duration: 8, aspectRatio: index_1.Seedance2AspectRatio.r16x9 } });
...
void imageClient.submit({ model: 'recraft_v4_1', prompt: { instruction: 'x' }, settings: { model: index_1.RecraftV41Model.vector, resolution: '2k', colors: ['#ffffff'] } });
...
void profileClient.getUser().then((user) => {
    if (user) {
        expectType(user.workspaceId);
        expectType(user.workspaceType);
        expectType(user.workspaceRole);
        expectType(user.planType);
        expectType(user.billingPeriod);
        expectType(user.totalPlanCredits);
        // @ts-expect-error profile user fields are camelCase in the SDK
        expectType(user.workspace_id);
    }
});
void profileClient.listWorkspaces().then((workspaces) => {
    expectType(workspaces[0]?.clerkOrganizationId);
    expectType(workspaces[0]?.avatarUrl);
    // @ts-expect-error workspace fields are camelCase in the SDK
    expectType(workspaces[0]?.clerk_organization_id);
});
void profileClient.getCredits({ includeOnDemand: true }).then((credits) => {
    expectType(credits?.totalAvailableCredits);
    expectType(credits?.raw.totalAvailableCredits);
});
void profileClient.switchWorkspace({ workspaceId: 'workspace-1' }).then((snapshot) => {
    expectType(snapshot.currentWorkspace?.id);
    expectType(snapshot.credits?.availablePercent);
});
...
const textOnly = (0, index_1.defineJob)({
    jobSetType: 'text_only',
    outputType: 'image',
    params: {
        settings: { steps: index_1.z.number() },
    },
    // credits receives the TYPED settings of this model
    credits: ({ settings }) => settings.steps * 2,
});
void (0, index_1.defineJob)({
    jobSetType: 'priced_t',
    outputType: 'image',
    params: { settings: { steps: index_1.z.number() } },
    // @ts-expect-error 'nope' is not a settings key of this model
    credits: ({ settings }) => settings.nope,
});
const textClient = (0, index_1.createJobClient)({ adapter: backend, jobs: [textOnly] });
...
//# sourceMappingURL=typed-api.test-d.js.map