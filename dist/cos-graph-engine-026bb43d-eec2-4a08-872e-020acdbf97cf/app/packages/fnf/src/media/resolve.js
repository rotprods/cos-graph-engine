...
function typeForUrl(url) {
    if (AUDIO_URL.test(url))
        return 'audio_input';
    return (0, selectors_1.getMediaType)(url) === 'video' ? 'video_input' : 'media_input';
}
async function resolveOne(ctx, ref) {
    ...
    if (ctx.resolveJob) {
        try {
            ...
            if (err instanceof errors_1.JobAbortedError)
                throw err;
        }
    }
    return { id: ref, type: 'media_input' };
}
async function resolveMedia(ctx, refs) {
    return (0, observability_1.observeAsync)(ctx.observability, 'fnf.media.resolve', { ref_count: refs.length }, () => Promise.all(refs.map(ref => resolveOne(ctx, ref))), {
