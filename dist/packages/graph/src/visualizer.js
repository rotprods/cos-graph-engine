...
exports.generateVisualizer = generateVisualizer;
exports.generateVisualizerFile = generateVisualizerFile;
...
const LEVEL_ENGINES = {
    L0: { name: 'Visual Graph', color: '#4A90D9', buildDemo: (e) => e.buildDemo() },
    L1: { name: 'Execution Graph', color: '#7B68EE', buildDemo: (e) => e.buildDemo() },
    L2: { name: 'State Machine', color: '#2ECC71', buildDemo: (e) => e.buildDemo() },
    L3: { name: 'Dependency Graph', color: '#E74C3C', buildDemo: (e) => e.buildDiamond() },
    L4: { name: 'Call Graph', color: '#F39C12', buildDemo: (e) => e.buildDemo() },
    L5: { name: 'CFG', color: '#1ABC9C', buildDemo: (e) => e.buildIfElse() },
    L6: { name: 'DataFlow', color: '#3498DB', buildDemo: (e) => e.buildETLPipeline() },
    L7: { name: 'Compute Graph', color: '#9B59B6', buildDemo: (e) => e.buildMLP() },
    L8: { name: 'Knowledge Graph', color: '#E67E22', buildDemo: (e) => e.buildKnowledgeGraph() },
    L9: { name: 'Semantic Graph', color: '#1ABC9C', buildDemo: (e) => e.buildDemo() },
    L10: { name: 'Embedding Graph', color: '#2980B9', buildDemo: (e) => e.buildDemo() },
    L11: { name: 'GraphRAG', color: '#8E44AD', buildDemo: (e) => e.buildDemo() },
    L12: { name: 'Memory Graph', color: '#16A085', buildDemo: (e) => e.buildMemoryGraph() },
    L13: { name: 'Agent Graph', color: '#27AE60', buildDemo: (e) => e.buildDevTeam() },
    L14: { name: 'Tool Graph', color: '#2C3E50', buildDemo: (e) => e.buildToolEcosystem() },
    L15: { name: 'Workflow Graph', color: '#D35400', buildDemo: (e) => e.buildWorkflow() },
    L16: { name: 'Network Graph', color: '#C0392B', buildDemo: (e) => e.buildInfrastructure() },
    L17: { name: 'Social Graph', color: '#E91E63', buildDemo: (e) => e.buildTechNetwork() },
    L18: { name: 'Biological Graph', color: '#00BCD4', buildDemo: (e) => e.buildNeuralCircuit() },
    L19: { name: 'Molecular Graph', color: '#FF5722', buildDemo: (e) => e.buildWater() },
};
function generateHTML(levels) {
...
function generateVisualizer(levels, config = {}) {
    ...
    for (const levelId of targetLevels) {
        ...
        try {
            ...
            data.push({
                ...
                name: info.name,
                color: info.color,
                nodes: nodes.map((n) => ({ id: n.id, label: n.label || n.name || n.element || n.type || n.id, type: n.type || n.role || '' })),
                edges: edges.map((e) => ({ source: e.source || e.from, target: e.target || e.to, type: e.type || e.event || '' })),
                metrics,
            });
        }
        catch (err) {
            data.push({ id: levelId, name: info.name, color: info.color, nodes: [], edges: [], metrics: { error: err.message } });
        }
    }
    return generateHTML(data);
}
function getEngineClass(levelId) {
    const map = {
        L0: level0_visual_1.VisualGraphEngine, L1: level1_execution_1.ExecutionGraphEngine, L2: level2_state_1.StateMachine, L3: level3_dependency_1.DependencyResolver,
        L4: level4_call_1.CallGraphBuilder, L5: level5_cfg_1.CFGBuilder, L6: level6_dataflow_1.DataFlowGraph, L7: level7_compute_1.ComputationalGraph,
        L8: level8_knowledge_1.KnowledgeGraphEngine, L9: level9_semantic_1.SemanticGraph, L10: level10_embedding_1.EmbeddingGraph, L11: level11_graphrag_1.GraphRAGEngine,
        L12: level12_memory_1.MemoryGraphEngine, L13: level13_agent_1.AgentGraphEngine, L14: level14_tool_1.ToolGraphEngine, L15: level15_workflow_1.WorkflowGraphEngine,
        L16: level16_network_1.NetworkGraphEngine, L17: level17_social_1.SocialGraphEngine, L18: level18_biological_1.BiologicalGraphEngine, L19: level19_molecular_1.MolecularGraphEngine,
    };
    return map[levelId];
}
function generateVisualizerFile(outputFile, levels, config = {}) {
