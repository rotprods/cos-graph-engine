...
exports.ComputationalGraph = void 0;
class ComputationalGraph {
    nodes = [];
    edges = [];
    values = new Map();
    gradients = new Map();
    addNode(n) { this.nodes.push(n); return n.id; }
     ...
     */
    buildMLP(inputDim = 784, hiddenDim = 256, numClasses = 2) {
        // Input leaf
    ...
    buildExpression() {
        ...
        this.addEdge({ source: 'x', target: 't1' });
        this.addEdge({ source: 'y', target: 't1' });
        this.addEdge({ source: 'w', target: 't2' });
        this.addEdge({ source: 'v', target: 't2' });
        this.addEdge({ source: 't1', target: 'z' });
        this.addEdge({ source: 't2', target: 'z' });
    }
    ...
    backward() {
        ...
        for (const nodeId of order) {
            ...
            if (Math.abs(grad) < 1e-10)
                continue; // skip zero-gradient nodes (dead branches)
    ...
    localGradient(op, upstreamGrad, inputs) {
        switch (op) {
            ...
            case 'tanh': {
                const t = Math.tanh(inputs[0] || 0);
                return [upstreamGrad * (1 - t * t)];
            }
            case 'sigmoid': {
                const s = 1 / (1 + Math.exp(-(inputs[0] || 0)));
                return [upstreamGrad * s * (1 - s)];
            }
    ...
    computeOp(op, inputs, params) {
        switch (op) {
            case 'constant':
                ...
                return inputs.length > 0 ? inputs[0] : (params?.value ?? 0);
    ...
    paramCount() { return this.nodes.filter(n => n.requiresGrad).length; }
    toMermaid() {
        let m = 'graph TD\n';
        for (const n of this.nodes) {
...
exports.ComputationalGraph = ComputationalGraph;
//# sourceMappingURL=level7-compute.js.map