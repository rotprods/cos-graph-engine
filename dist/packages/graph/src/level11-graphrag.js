...
class GraphRAGEngine {
    ...
    addChunk(c) {
        if (this.chunks.some(x => x.id === c.id))
            throw new Error(`Duplicate chunk ID: ${c.id}`);
        this.chunks.push(c);
    }
    addEntity(id, name, type = 'concept') {
    ...
    addRelation(source, target, type = 'related_to') {
        if (!this.entities.some(e => e.id === source))
            throw new Error(`Relation source ${source} not found`);
        if (!this.entities.some(e => e.id === target))
            throw new Error(`Relation target ${target} not found`);
        this.relations.push({ id: (0, core_1.generateId)(), source, target, type });
        this.buildAdjacency();
    }
    ...
    retrieve(queryEmbedding, queryEntities = []) {
        ...
        for (const eid of walkQueue) {
            const dfs = (id, depth) => {
                if (depth > this.config.walkDepth || visitedEntities.has(id))
                    return;
        ...
        const resultEntities = Array.from(visitedEntities).map(id => this.entities.find(e => e.id === id)).filter(Boolean);
        return {
    ...
    async answer(query, queryEmbedding, queryEntities = []) {
...
//# sourceMappingURL=level11-graphrag.js.map