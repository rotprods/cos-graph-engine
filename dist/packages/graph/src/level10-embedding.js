...
class EmbeddingGraph {
    ...
    addNode(n) {
        if (this.nodes.some(x => x.id === n.id))
            throw new Error(`Duplicate embedding node ID: ${n.id}`);
        this.nodes.push(n);
        this.buildAdjacency();
        return n.id;
    }
    ...
    buildKNN(k = 3) {
        ...
        for (let i = 0; i < this.nodes.length; i++) {
            ...
            for (let j = 1; j <= Math.min(k, dists.length - 1); j++) {
                ...
                this.edges.push({ id: (0, core_1.generateId)(), source: this.nodes[i].id, target: target.id, similarity: 1 / (1 + dists[j].dist), distance: dists[j].dist });
            }
        }
        this.buildAdjacency();
    }
    buildEpsilon(epsilon = 0.5) {
        this.edges = [];
        for (let i = 0; i < this.nodes.length; i++) {
            for (let j = i + 1; j < this.nodes.length; j++) {
                const dist = EmbeddingGraph.distance(this.nodes[i].vector, this.nodes[j].vector);
                if (dist < epsilon) {
                    this.edges.push({ id: (0, core_1.generateId)(), source: this.nodes[i].id, target: this.nodes[j].id, similarity: 1 / (1 + dist), distance: dist });
                }
            }
        }
        this.buildAdjacency();
    ...
    cluster(k = 3, seed) {
        ...
        for (let c = 1; c < k; c++) {
            ...
            for (let i = 0; i < n; i++) {
                r -= dists[i];
                if (r <= 0) {
                    centroids.push(this.nodes[i].vector.slice());
                    break;
                }
            }
        }
        ...
        for (let iter = 0; iter < 50; iter++) {
            ...
            for (let i = 0; i < n; i++) {
                for (let d = 0; d < dim; d++)
                    centroids[assignments[i]][d] += this.nodes[i].vector[d];
...
exports.EmbeddingGraph = EmbeddingGraph;
//# sourceMappingURL=level10-embedding.js.map