...
exports.AgentGraphEngine = void 0;
const core_1 = require("@cos/core");
class AgentGraphEngine {
    graph;
    adj = new Map();
    constructor(name = 'Agent Swarm') {
    ...
    delegationChain(fromId, toId) {
        this.buildAdjacency();
        const visited = new Set();
        const dfs = (id, path) => {
            if (id === toId)
                return [...path, this.graph.nodes.find(n => n.id === id)];
            if (visited.has(id))
                return null;
            visited.add(id);
    ...
    static fromJSON(data) { const g = new AgentGraphEngine(data.name); g.graph = data; g.buildAdjacency(); return g; }
    toMermaid() {
        let m = 'graph TD\n';
        for (const n of this.graph.nodes) {
...
exports.AgentGraphEngine = AgentGraphEngine;
//# sourceMappingURL=level13-agent.js.map