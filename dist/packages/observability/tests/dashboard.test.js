...
const dashboard_1 = require("../src/dashboard");
// ============================================================
// Helpers
// ============================================================
let passed = 0;
...
function makeCollectorWithTraces(n) {
    const c = new src_1.TraceCollectorImpl(100);
    for (let i = 0; i < n; i++) {
        const s = new src_1.TraceSessionImpl();
...
section('ExportService — JSON');
{
    ...
    const svc = new dashboard_1.ExportService(collector);
...
{
    const collector = new src_1.TraceCollectorImpl(10);
    const svc = new dashboard_1.ExportService(collector);
...
(async () => {
    const collector = new src_1.TraceCollectorImpl(100);
    const dashboard = new dashboard_1.TelemetryDashboard({ port: 0, collector });
...
{
    const exporter = new dashboard_1.OTLPExporter({
