...
function assertNear(a, b, tolerance, msg) {
    testCount++;
...
{
    const session = new tracing_1.TraceSessionImpl(true);
    session.addHop({ hopIndex: 0, nodeId: 'a', depth: 0, duration: 1, source: 'forward' });
    const summary = session.getSummary();
...
{
    const a = tracing_1.NoopTraceSession.instance;
    const b = tracing_1.NoopTraceSession.instance;
...
{
    const session = tracing_1.NoopTraceSession.instance;
...
section('formatTraceSummary');
{
    ...
    const text = (0, tracing_1.formatTraceSummary)(summary);
...
section('CSR + NoopTraceSession — zero overhead');
{
    ...
    const noop = tracing_1.NoopTraceSession.instance;
    // Warmup
