...
{
    const buf = new collector_1.CircularBuffer(5);
...
{
    const buf = new collector_1.CircularBuffer(3);
    buf.push(1);
    buf.push(2);
    buf.push(3);
...
{
    try {
        new collector_1.CircularBuffer(0);
    ...
    try {
        new collector_1.CircularBuffer(-1);
...
{
    const buf = new collector_1.CircularBuffer(1);
...
{
    const collector = new collector_1.TraceCollectorImpl(10);
    assert(collector.isEmpty, 'initially empty');
    assert(collector.size === 0, 'size 0');
    const session = new tracing_1.TraceSessionImpl();
...
section('TraceCollectorImpl — query methods');
{
    const collector = new collector_1.TraceCollectorImpl(10);
    for (let i = 0; i < 5; i++) {
        const s = new tracing_1.TraceSessionImpl();
...
{
    const collector = new collector_1.TraceCollectorImpl(3);
    for (let i = 0; i < 5; i++) {
        const s = new tracing_1.TraceSessionImpl();
        s.addHop({ hopIndex: 0, nodeId: `n${i}`, depth: 0, duration: 1, source: 'forward' });
        collector.record(s);
...
{
    const a = collector_1.NoopTraceCollector.instance;
    const b = collector_1.NoopTraceCollector.instance;
...
section('Integration: CSR BFS + collector');
{
    ...
    const collector = new collector_1.TraceCollectorImpl(100);
...
{
    const c1 = new collector_1.TraceCollectorImpl(10);
    const c2 = new collector_1.TraceCollectorImpl(10);
    const s1 = new tracing_1.TraceSessionImpl();
    s1.addHop({ hopIndex: 0, nodeId: 'a', depth: 0, duration: 1, source: 'forward' });
    c1.record(s1);
    const s2 = new tracing_1.TraceSessionImpl();
    s2.addHop({ hopIndex: 0, nodeId: 'b', depth: 0, duration: 1, source: 'forward' });
    c2.record(s2);
    const merged = (0, collector_1.mergeCollectorsExport)([c1, c2]);
