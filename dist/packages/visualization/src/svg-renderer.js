...
exports.SVGGraphRenderer = exports.ForceLayout = void 0;
// ============================================================
...
class ForceLayout {
    ...
    compute(graph, iterations = 100, nodeRadius = 20) {
...
class SVGGraphRenderer {
    ...
    _computeLayout(graph, layout, width, height, iterations, nodeRadius) {
        switch (layout) {
            case 'force':
    ...
    _treeLayout(graph, width, height) {
    ...
    _radialLayout(graph, width, height) {
        const positions = new Map();
        const nodes = graph.getAllNodes();
        if (nodes.length === 0)
            return positions;
